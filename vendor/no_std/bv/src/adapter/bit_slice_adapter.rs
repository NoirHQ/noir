...
macro_rules! impl_bit_sliceable_adapter {
    (
        ...
        $(
            impl<$($param)*> $crate::BitSliceable<::core::ops::Range<u64>> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, range: ::core::ops::Range<u64>) -> Self::Slice {
                    assert!( range.start <= range.end,
                             "{}::slice: bad range", stringify!($target) );
                    $crate::adapter::BitSliceAdapter::new(self, range.start, range.end - range.start)
                }
            }

            impl<$($param)*> $crate::BitSliceable<::core::ops::RangeFrom<u64>> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, range: ::core::ops::RangeFrom<u64>) -> Self::Slice {
                    let len = self.bit_len();
                    self.bit_slice(range.start .. len)
                }
            }

            impl<$($param)*> $crate::BitSliceable<::core::ops::RangeTo<u64>> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, range: ::core::ops::RangeTo<u64>) -> Self::Slice {
                    $crate::adapter::BitSliceAdapter::new(self, 0, range.end)
                }
            }

            impl<$($param)*> $crate::BitSliceable<::core::ops::RangeFull> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, _range: ::core::ops::RangeFull) -> Self::Slice {
                    let len = self.bit_len();
                    $crate::adapter::BitSliceAdapter::new(self, 0, len)
                }
            }

            impl<$($param)*> $crate::BitSliceable<::core::ops::RangeInclusive<u64>> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, range: ::core::ops::RangeInclusive<u64>) -> Self::Slice {
                    let (start, end) = $crate::range_compat::get_inclusive_bounds(range)
                        .expect("BitSliceable::bit_slice: bad inclusive range");
                    $crate::adapter::BitSliceAdapter::new(self, start, end - start + 1)
                }
            }

            impl<$($param)*> $crate::BitSliceable<::core::ops::RangeToInclusive<u64>> for $target {
                type Slice = $crate::adapter::BitSliceAdapter<Self>;

                fn bit_slice(self, range: ::core::ops::RangeToInclusive<u64>) -> Self::Slice {
                    $crate::adapter::BitSliceAdapter::new(self, 0, range.end + 1)
                }
            }
        )+
    };
...
impl<T: Bits> Bits for BitSliceAdapter<T> {
    ...
    fn get_block(&self, position: usize) -> Self::Block {
        ...
        let (real_start, real_len) = get_block_addr::<T::Block>(self.start, self.len, position);
        self.bits.get_bits(real_start, real_len)
    }

    fn get_bits(&self, start: u64, count: usize) -> Self::Block {
        assert!(
            start + count as u64 <= self.bit_len(),
...
fn get_block_addr<Block: BlockType>(start: u64, len: u64, position: usize) -> (u64, usize) {
...
impl<T: Bits> BitSliceable<RangeInclusive<u64>> for BitSliceAdapter<T> {
    ...
    fn bit_slice(self, range: RangeInclusive<u64>) -> Self::Slice {
        let (start, limit) =
            get_inclusive_bounds(range).expect("BitSliceAdapter::bit_slice: bad range");
