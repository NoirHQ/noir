...
use crate::{
	prelude::*, Coin, CoinsError, OverflowError, OverflowOperation, StdError, StdResult, Uint128,
};
...
impl FromStr for Coins {
	...
	fn from_str(s: &str) -> StdResult<Self> {
		...
		Ok(s.split(',').map(Coin::from_str).collect::<Result<Vec<_>, _>>()?.try_into()?)
	}
}

impl fmt::Display for Coins {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let s = self.0.values().map(|coin| coin.to_string()).collect::<Vec<_>>().join(",");
		write!(f, "{s}")
	}
}
...
impl Coins {
	...
	pub fn amount_of(&self, denom: &str) -> Uint128 {
		self.0.get(denom).map(|c| c.amount).unwrap_or_else(Uint128::zero)
	}
...
impl Iterator for CoinsIntoIter {
	...
	fn size_hint(&self) -> (usize, Option<usize>) {
		// Since btree_map::IntoIter implements ExactSizeIterator, this is guaranteed to return the
		// exact length
		self.0.size_hint()
	}
}
...
impl<'a> Iterator for CoinsIter<'a> {
	...
	fn size_hint(&self) -> (usize, Option<usize>) {
		// Since btree_map::Iter implements ExactSizeIterator, this is guaranteed to return the
		// exact length
		self.0.size_hint()
	}
}
...
mod tests {
	...
	fn handling_zero_amount() {
		...
		assert_ne!(coins.amount_of("factory/osmo1234abcd/subdenom"), Uint128::zero());
	...
	fn can_iterate_borrowed() {
		let coins = mock_coins();
		assert!(coins.iter().map(|c| &c.denom).eq(coins.to_vec().iter().map(|c| &c.denom)));
