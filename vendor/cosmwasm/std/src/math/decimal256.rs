...
use crate::{
	...
	Decimal, SignedDecimal, SignedDecimal256, Uint512,
...
impl Decimal256 {
	...
	pub fn from_atomics(
		...
		Ok(match decimal_places.cmp(&Self::DECIMAL_PLACES) {
			Ordering::Less => {
				...
				Self(atomics.checked_mul(factor).map_err(|_| Decimal256RangeExceeded)?)
			},
	...
	pub fn checked_ceil(&self) -> Result<Self, RoundUpOverflowError> {
		...
		if floor == self {
			...
			floor.checked_add(Decimal256::one()).map_err(|_| RoundUpOverflowError)
		}
	}
	...
	/// Multiplies one `Decimal256` by another, returning an `OverflowError` if an overflow
	/// occurred.
	...
	pub fn sqrt(&self) -> Self {
		...
		(0..=Self::DECIMAL_PLACES / 2)
			...
			// The last step (i = 0) is guaranteed to succeed because `isqrt(Uint256::MAX) * 10^9`
			// does not overflow
			.unwrap()
	}
...
impl TryFrom<SignedDecimal> for Decimal256 {
	...
	fn try_from(value: SignedDecimal) -> Result<Self, Self::Error> {
		value.atomics().try_into().map(Decimal256).map_err(|_| Decimal256RangeExceeded)
	}
}
	...
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		if fractional.is_zero() {
			...
			let fractional_string =
				format!("{:0>padding$}", fractional, padding = Self::DECIMAL_PLACES as usize);
...
mod tests {
	...
	fn decimal256_from_atomics_works() {
		...
		assert_eq!(Decimal256::from_atomics(1000000000000000000u128, 18).unwrap(), one);
		assert_eq!(Decimal256::from_atomics(10000000000000000000u128, 19).unwrap(), one);
		assert_eq!(Decimal256::from_atomics(100000000000000000000u128, 20).unwrap(), one);
		...
		assert_eq!(Decimal256::from_atomics(2000000000000000000u128, 18).unwrap(), two);
		assert_eq!(Decimal256::from_atomics(20000000000000000000u128, 19).unwrap(), two);
		assert_eq!(Decimal256::from_atomics(200000000000000000000u128, 20).unwrap(), two);
		...
		assert_eq!(Decimal256::from_atomics(max.atomics(), max.decimal_places()).unwrap(), max);
	...
	fn decimal256_from_ratio_works() {
		...
		// 1.5
		assert_eq!(Decimal256::from_ratio(3u128, 2u128), Decimal256::percent(150));
		assert_eq!(Decimal256::from_ratio(150u128, 100u128), Decimal256::percent(150));
		assert_eq!(Decimal256::from_ratio(333u128, 222u128), Decimal256::percent(150));

		// 0.125
		assert_eq!(Decimal256::from_ratio(1u64, 8u64), Decimal256::permille(125));
		assert_eq!(Decimal256::from_ratio(125u64, 1000u64), Decimal256::permille(125));
		...
		assert_eq!(Decimal256::from_ratio(u128::MAX, u128::MAX), Decimal256::one());
	...
	fn decimal256_implements_fraction() {
		...
		assert_eq!(fraction.numerator(), Uint256::from_str("1234567000000000000000").unwrap());
		assert_eq!(fraction.denominator(), Uint256::from_str("1000000000000000000").unwrap());
	}
	...
	fn decimal256_from_str_works() {
		...
		assert_eq!(Decimal256::from_str("42").unwrap(), Decimal256::percent(4200));
		assert_eq!(Decimal256::from_str("000").unwrap(), Decimal256::percent(0));
		assert_eq!(Decimal256::from_str("001").unwrap(), Decimal256::percent(100));
		assert_eq!(Decimal256::from_str("005").unwrap(), Decimal256::percent(500));
		assert_eq!(Decimal256::from_str("0042").unwrap(), Decimal256::percent(4200));

		// Decimals
		assert_eq!(Decimal256::from_str("1.0").unwrap(), Decimal256::percent(100));
		assert_eq!(Decimal256::from_str("1.5").unwrap(), Decimal256::percent(150));
		assert_eq!(Decimal256::from_str("0.5").unwrap(), Decimal256::percent(50));
		assert_eq!(Decimal256::from_str("0.123").unwrap(), Decimal256::permille(123));

		assert_eq!(Decimal256::from_str("40.00").unwrap(), Decimal256::percent(4000));
		assert_eq!(Decimal256::from_str("04.00").unwrap(), Decimal256::percent(400));
		assert_eq!(Decimal256::from_str("00.40").unwrap(), Decimal256::percent(40));
		assert_eq!(Decimal256::from_str("00.04").unwrap(), Decimal256::percent(4));
	...
	fn decimal256_add_works() {
		...
		assert_eq!(Decimal256::percent(5) + Decimal256::percent(4), Decimal256::percent(9));
		assert_eq!(Decimal256::percent(5) + Decimal256::zero(), Decimal256::percent(5));
	...
	fn decimal256_sub_works() {
		...
		assert_eq!(Decimal256::percent(9) - Decimal256::percent(4), Decimal256::percent(5));
	...
	fn decimal256_checked_mul() {
		let test_data = [
			...
			(Decimal256::MAX / Uint256::from_uint128(2u128.into()), Decimal256::percent(200)),
	...
	fn decimal256_implements_div() {
		...
		assert_eq!(Decimal256::percent(15) / Decimal256::percent(60), Decimal256::percent(25));
	...
	fn decimal256_checked_pow() {
		for exp in 0..10 {
			assert_eq!(Decimal256::one().checked_pow(exp).unwrap(), Decimal256::one());
		}
		...
		assert_eq!(Decimal256::zero().checked_pow(0).unwrap(), Decimal256::one());

		for exp in 1..10 {
			assert_eq!(Decimal256::zero().checked_pow(exp).unwrap(), Decimal256::zero());
		}

		for num in &[Decimal256::percent(50), Decimal256::percent(99), Decimal256::percent(200)] {
			assert_eq!(num.checked_pow(0).unwrap(), Decimal256::one())
		}

		assert_eq!(Decimal256::percent(20).checked_pow(2).unwrap(), Decimal256::percent(4));

		assert_eq!(Decimal256::percent(20).checked_pow(3).unwrap(), Decimal256::permille(8));

		assert_eq!(Decimal256::percent(200).checked_pow(4).unwrap(), Decimal256::percent(1600));

		assert_eq!(Decimal256::percent(200).checked_pow(4).unwrap(), Decimal256::percent(1600));

		assert_eq!(Decimal256::percent(700).checked_pow(5).unwrap(), Decimal256::percent(1680700));
		...
		assert_eq!(Decimal256::percent(10).checked_pow(18).unwrap(), Decimal256(1u128.into()));
	}

	#[test]
	fn decimal256_checked_pow_overflow() {
		assert_eq!(Decimal256::MAX.checked_pow(2), Err(OverflowError::new(OverflowOperation::Pow)));
	}
	...
	fn decimal256_to_string() {
		...
		assert_eq!(Decimal256(Uint256::from(1u128)).to_string(), "0.000000000000000001");
		assert_eq!(Decimal256(Uint256::from(10u128)).to_string(), "0.00000000000000001");
		assert_eq!(Decimal256(Uint256::from(100u128)).to_string(), "0.0000000000000001");
		assert_eq!(Decimal256(Uint256::from(1000u128)).to_string(), "0.000000000000001");
		assert_eq!(Decimal256(Uint256::from(10000u128)).to_string(), "0.00000000000001");
		assert_eq!(Decimal256(Uint256::from(100000u128)).to_string(), "0.0000000000001");
		assert_eq!(Decimal256(Uint256::from(1000000u128)).to_string(), "0.000000000001");
		assert_eq!(Decimal256(Uint256::from(10000000u128)).to_string(), "0.00000000001");
		assert_eq!(Decimal256(Uint256::from(100000000u128)).to_string(), "0.0000000001");
		assert_eq!(Decimal256(Uint256::from(1000000000u128)).to_string(), "0.000000001");
		assert_eq!(Decimal256(Uint256::from(10000000000u128)).to_string(), "0.00000001");
		assert_eq!(Decimal256(Uint256::from(100000000000u128)).to_string(), "0.0000001");
		assert_eq!(Decimal256(Uint256::from(10000000000000u128)).to_string(), "0.00001");
		assert_eq!(Decimal256(Uint256::from(100000000000000u128)).to_string(), "0.0001");
		assert_eq!(Decimal256(Uint256::from(1000000000000000u128)).to_string(), "0.001");
		assert_eq!(Decimal256(Uint256::from(10000000000000000u128)).to_string(), "0.01");
		assert_eq!(Decimal256(Uint256::from(100000000000000000u128)).to_string(), "0.1");
	}
	...
	fn decimal256_iter_sum() {
		...
		assert_eq!(items.iter().sum::<Decimal256>(), Decimal256::from_str("4").unwrap());
		assert_eq!(items.into_iter().sum::<Decimal256>(), Decimal256::from_str("4").unwrap());
	...
	fn decimal256_serialize() {
		...
		assert_eq!(serde_json::to_vec(&Decimal256::percent(8)).unwrap(), br#""0.08""#);
		assert_eq!(serde_json::to_vec(&Decimal256::percent(87)).unwrap(), br#""0.87""#);
		assert_eq!(serde_json::to_vec(&Decimal256::percent(876)).unwrap(), br#""8.76""#);
		assert_eq!(serde_json::to_vec(&Decimal256::percent(8765)).unwrap(), br#""87.65""#);
	}

	#[test]
	fn decimal256_deserialize() {
		assert_eq!(serde_json::from_slice::<Decimal256>(br#""0""#).unwrap(), Decimal256::zero());
		assert_eq!(serde_json::from_slice::<Decimal256>(br#""1""#).unwrap(), Decimal256::one());
		assert_eq!(serde_json::from_slice::<Decimal256>(br#""000""#).unwrap(), Decimal256::zero());
		assert_eq!(serde_json::from_slice::<Decimal256>(br#""001""#).unwrap(), Decimal256::one());
	...
	fn decimal256_rem_works() {
		// 4.02 % 1.11 = 0.69
		assert_eq!(Decimal256::percent(402) % Decimal256::percent(111), Decimal256::percent(69));

		// 15.25 % 4 = 3.25
		assert_eq!(Decimal256::percent(1525) % Decimal256::percent(400), Decimal256::percent(325));
	...
	fn decimal256_checked_methods() {
		// checked add
		assert_eq!(
			Decimal256::percent(402).checked_add(Decimal256::percent(111)).unwrap(),
		...
		// checked sub
		assert_eq!(
			Decimal256::percent(1111).checked_sub(Decimal256::percent(111)).unwrap(),
		...
		// checked div
		assert_eq!(
			Decimal256::percent(30).checked_div(Decimal256::percent(200)).unwrap(),
			Decimal256::percent(15)
		);
		assert_eq!(
			Decimal256::percent(88).checked_div(Decimal256::percent(20)).unwrap(),
		...
		// checked rem
		assert_eq!(
			Decimal256::percent(402).checked_rem(Decimal256::percent(111)).unwrap(),
			Decimal256::percent(69)
		);
		assert_eq!(
			Decimal256::percent(1525).checked_rem(Decimal256::percent(400)).unwrap(),
	...
	fn decimal256_pow_works() {
		...
		assert_eq!(Decimal256::percent(200).pow(10), Decimal256::percent(102400));
	}
	...
	fn decimal256_saturating_works() {
		...
		assert_eq!(Decimal256::MAX.saturating_add(Decimal256::percent(200)), Decimal256::MAX);
		assert_eq!(
			Decimal256::percent(200).saturating_sub(Decimal256::percent(100)),
			Decimal256::percent(100)
		);
		assert_eq!(Decimal256::zero().saturating_sub(Decimal256::percent(200)), Decimal256::zero());
		assert_eq!(
		...
		assert_eq!(Decimal256::percent(400).saturating_pow(2u32), Decimal256::percent(1600));
	...
	fn decimal256_checked_ceil() {
		assert_eq!(Decimal256::percent(199).checked_ceil(), Ok(Decimal256::percent(200)));
	...
	fn decimal256_to_uint_floor_works() {
		...
		let tests = vec![
			(Decimal256::from_str("12.345").unwrap(), Uint256::from(12u128)),
			(Decimal256::from_str("0.98451384").unwrap(), Uint256::from(0u128)),
			(Decimal256::from_str("178.0").unwrap(), Uint256::from(178u128)),
			(Decimal256::MIN, Uint256::from(0u128)),
			(Decimal256::MAX, Uint256::MAX / Decimal256::DECIMAL_FRACTIONAL),
		];
