...
impl Int64 {
	...
	pub fn full_mul(self, rhs: impl Into<Self>) -> Int128 {
		Int128::from(self).checked_mul(Int128::from(rhs.into())).unwrap()
	}
...
mod tests {
	...
	fn int64_wrapping_methods() {
		// wrapping_add
		assert_eq!(Int64::from(2u32).wrapping_add(Int64::from(2u32)), Int64::from(4u32)); // non-wrapping
		assert_eq!(Int64::MAX.wrapping_add(Int64::from(1u32)), Int64::MIN); // wrapping

		// wrapping_sub
		assert_eq!(Int64::from(7u32).wrapping_sub(Int64::from(5u32)), Int64::from(2u32)); // non-wrapping
		assert_eq!(Int64::MIN.wrapping_sub(Int64::from(1u32)), Int64::MAX); // wrapping

		// wrapping_mul
		assert_eq!(Int64::from(3u32).wrapping_mul(Int64::from(2u32)), Int64::from(6u32)); // non-wrapping
		assert_eq!(Int64::MAX.wrapping_mul(Int64::from(2u32)), Int64::from(-2i32)); // wrapping
	...
	fn int64_checked_multiply_ratio_works() {
		...
		assert_eq!(base.checked_multiply_ratio(654321i64, 654321i64).unwrap(), base);
		assert_eq!(base.checked_multiply_ratio(i64::MAX, i64::MAX).unwrap(), base);

		// factor 3/2
		assert_eq!(base.checked_multiply_ratio(3i64, 2i64).unwrap(), Int64(750));
		assert_eq!(base.checked_multiply_ratio(333333i64, 222222i64).unwrap(), Int64(750));

		// factor 2/3 (integer devision always floors the result)
		assert_eq!(base.checked_multiply_ratio(2i64, 3i64).unwrap(), Int64(333));
		assert_eq!(base.checked_multiply_ratio(222222i64, 333333i64).unwrap(), Int64(333));

		// factor 5/6 (integer devision always floors the result)
		assert_eq!(base.checked_multiply_ratio(5i64, 6i64).unwrap(), Int64(416));
		assert_eq!(base.checked_multiply_ratio(100i64, 120i64).unwrap(), Int64(416));
	}
	...
	fn sum_works() {
		let nums =
			vec![Int64::from(17u32), Int64::from(123u32), Int64::from(540u32), Int64::from(82u32)];
	...
	fn int64_methods() {
		// checked_*
		assert!(matches!(Int64::MAX.checked_add(Int64::from(1u32)), Err(OverflowError { .. })));
		assert_eq!(Int64::from(1u32).checked_add(Int64::from(1u32)), Ok(Int64::from(2u32)),);
		assert!(matches!(Int64::MIN.checked_sub(Int64::from(1u32)), Err(OverflowError { .. })));
		assert_eq!(Int64::from(2u32).checked_sub(Int64::from(1u32)), Ok(Int64::from(1u32)),);
		assert!(matches!(Int64::MAX.checked_mul(Int64::from(2u32)), Err(OverflowError { .. })));
		assert_eq!(Int64::from(2u32).checked_mul(Int64::from(2u32)), Ok(Int64::from(4u32)),);
		assert!(matches!(Int64::MAX.checked_pow(2u32), Err(OverflowError { .. })));
		assert_eq!(Int64::from(2u32).checked_pow(3u32), Ok(Int64::from(8u32)),);
		assert_eq!(Int64::MAX.checked_div(Int64::from(0u32)), Err(DivisionError::DivideByZero));
		assert_eq!(Int64::from(6u32).checked_div(Int64::from(2u32)), Ok(Int64::from(3u32)),);
		assert_eq!(
			Int64::MAX.checked_div_euclid(Int64::from(0u32)),
			Err(DivisionError::DivideByZero)
		);
		assert_eq!(Int64::from(6u32).checked_div_euclid(Int64::from(2u32)), Ok(Int64::from(3u32)),);
		assert_eq!(Int64::from(7u32).checked_div_euclid(Int64::from(2u32)), Ok(Int64::from(3u32)),);
		assert!(matches!(Int64::MAX.checked_rem(Int64::from(0u32)), Err(DivideByZeroError { .. })));
		// checked_* with negative numbers
		assert_eq!(Int64::from(-12i32).checked_div(Int64::from(10i32)), Ok(Int64::from(-1i32)),);
		assert_eq!(Int64::from(-2i32).checked_pow(3u32), Ok(Int64::from(-8i32)),);
		assert_eq!(Int64::from(-6i32).checked_mul(Int64::from(-7i32)), Ok(Int64::from(42i32)),);
		assert_eq!(Int64::from(-2i32).checked_add(Int64::from(3i32)), Ok(Int64::from(1i32)),);
		assert_eq!(
	...
	fn int64_shr() {
		...
		assert_eq!(Int64::MIN >> (core::mem::size_of::<Int64>() as u32 * 8 - 1), -Int64::one());
	}
	...
	fn int64_shl() {
		...
		assert_eq!(Int64::one() << (core::mem::size_of::<Int64>() as u32 * 8 - 1), Int64::MIN);
	}
	...
	fn int64_unsigned_abs_works() {
		...
		assert_eq!(Int64::MIN.unsigned_abs(), Uint64::new(Int64::MAX.0 as u64) + Uint64::one());
