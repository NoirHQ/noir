...
use crate::{
	...
	Decimal256, SignedDecimal, SignedDecimal256,
...
/// A fixed-point decimal value with 18 fractional digits, i.e. Decimal(1_000_000_000_000_000_000)
/// == 1.0
///
...
impl Decimal {
	...
	pub fn from_atomics(
		...
		Ok(match decimal_places.cmp(&Self::DECIMAL_PLACES) {
			Ordering::Less => {
				...
				Self(atomics.checked_mul(factor).map_err(|_| DecimalRangeExceeded)?)
			},
	...
	pub fn checked_ceil(&self) -> Result<Self, RoundUpOverflowError> {
		...
		if floor == self {
			...
			floor.checked_add(Decimal::one()).map_err(|_| RoundUpOverflowError)
		}
	}
	...
	pub fn sqrt(&self) -> Self {
		...
		(0..=Self::DECIMAL_PLACES / 2)
			...
			// The last step (i = 0) is guaranteed to succeed because `isqrt(u128::MAX) * 10^9` does
			// not overflow
			.unwrap()
	}
...
impl TryFrom<Decimal256> for Decimal {
	...
	fn try_from(value: Decimal256) -> Result<Self, Self::Error> {
		value.atomics().try_into().map(Decimal).map_err(|_| DecimalRangeExceeded)
	}
}
...
mod tests {
	...
	fn decimal_from_decimal256_works() {
		...
		assert_eq!(Decimal::try_from(Decimal256::percent(50)), Ok(Decimal::percent(50)));
	}
	...
	fn decimal_try_from_signed_works() {
		...
		assert_eq!(Decimal::try_from(SignedDecimal::zero()).unwrap(), Decimal::zero());
		assert_eq!(Decimal::try_from(SignedDecimal::one()).unwrap(), Decimal::one());
		assert_eq!(Decimal::try_from(SignedDecimal::percent(50)).unwrap(), Decimal::percent(50));
		assert_eq!(Decimal::try_from(SignedDecimal::negative_one()), Err(DecimalRangeExceeded));
	...
	fn decimal_from_atomics_works() {
		...
		assert_eq!(Decimal::from_atomics(1000000000000000000u128, 18).unwrap(), one);
		assert_eq!(Decimal::from_atomics(10000000000000000000u128, 19).unwrap(), one);
		assert_eq!(Decimal::from_atomics(100000000000000000000u128, 20).unwrap(), one);
		...
		assert_eq!(Decimal::from_atomics(2000000000000000000u128, 18).unwrap(), two);
		assert_eq!(Decimal::from_atomics(20000000000000000000u128, 19).unwrap(), two);
		assert_eq!(Decimal::from_atomics(200000000000000000000u128, 20).unwrap(), two);
		...
		let max = Decimal::MAX;
		assert_eq!(Decimal::from_atomics(max.atomics(), max.decimal_places()).unwrap(), max);
	...
	fn decimal_implements_fraction() {
		...
		assert_eq!(fraction.numerator(), Uint128::from(1_234_567_000_000_000_000_000u128));
		assert_eq!(fraction.denominator(), Uint128::from(1_000_000_000_000_000_000u128));
	}
	...
	fn decimal_inv_works() {
		...
		// d > 1 exact
		assert_eq!(Decimal::from_str("2").unwrap().inv(), Some(Decimal::from_str("0.5").unwrap()));
		assert_eq!(
		...
		// d < 1 exact
		assert_eq!(Decimal::from_str("0.5").unwrap().inv(), Some(Decimal::from_str("2").unwrap()));
		assert_eq!(
	...
	fn decimal_add_works() {
		...
		assert_eq!(value.0, Decimal::DECIMAL_FRACTIONAL * Uint128::from(3u8) / Uint128::from(2u8));

		assert_eq!(Decimal::percent(5) + Decimal::percent(4), Decimal::percent(9));
	...
	fn decimal_sub_works() {
		...
		assert_eq!(Decimal::percent(9) - Decimal::percent(4), Decimal::percent(5));
	...
	fn decimal_implements_mul() {
		...
		// Move right
		let max = Decimal::MAX;
		assert_eq!(max * dec("1.0"), dec("340282366920938463463.374607431768211455"));
		assert_eq!(max * dec("0.1"), dec("34028236692093846346.337460743176821145"));
		assert_eq!(max * dec("0.01"), dec("3402823669209384634.633746074317682114"));
		assert_eq!(max * dec("0.001"), dec("340282366920938463.463374607431768211"));
		assert_eq!(max * dec("0.000001"), dec("340282366920938.463463374607431768"));
		assert_eq!(max * dec("0.000000001"), dec("340282366920.938463463374607431"));
		assert_eq!(max * dec("0.000000000001"), dec("340282366.920938463463374607"));
		assert_eq!(max * dec("0.000000000000001"), dec("340282.366920938463463374"));
		assert_eq!(max * dec("0.000000000000000001"), dec("340.282366920938463463"));
	...
	fn decimal_implements_div() {
		...
		assert_eq!(Decimal::percent(15) / Decimal::percent(60), Decimal::percent(25));
	...
	fn decimal_uint128_sqrt_does_not_overflow() {
		assert_eq!(Decimal::from_str("400").unwrap().sqrt(), Decimal::from_str("20").unwrap());
	}
	...
	fn decimal_checked_pow() {
		...
		for num in &[Decimal::percent(50), Decimal::percent(99), Decimal::percent(200)] {
			assert_eq!(num.checked_pow(0).unwrap(), Decimal::one())
		}

		assert_eq!(Decimal::percent(20).checked_pow(2).unwrap(), Decimal::percent(4));

		assert_eq!(Decimal::percent(20).checked_pow(3).unwrap(), Decimal::permille(8));

		assert_eq!(Decimal::percent(200).checked_pow(4).unwrap(), Decimal::percent(1600));

		assert_eq!(Decimal::percent(200).checked_pow(4).unwrap(), Decimal::percent(1600));

		assert_eq!(Decimal::percent(700).checked_pow(5).unwrap(), Decimal::percent(1680700));

		assert_eq!(Decimal::percent(700).checked_pow(8).unwrap(), Decimal::percent(576480100));

		assert_eq!(Decimal::percent(700).checked_pow(10).unwrap(), Decimal::percent(28247524900));
		...
		assert_eq!(Decimal::percent(10).checked_pow(18).unwrap(), Decimal(1u128.into()));
	}

	#[test]
	fn decimal_checked_pow_overflow() {
		assert_eq!(Decimal::MAX.checked_pow(2), Err(OverflowError::new(OverflowOperation::Pow)));
	}
	...
	fn decimal_to_string() {
		...
		assert_eq!(Decimal(Uint128::from(1u128)).to_string(), "0.000000000000000001");
		assert_eq!(Decimal(Uint128::from(10u128)).to_string(), "0.00000000000000001");
		assert_eq!(Decimal(Uint128::from(100u128)).to_string(), "0.0000000000000001");
		assert_eq!(Decimal(Uint128::from(1000u128)).to_string(), "0.000000000000001");
		assert_eq!(Decimal(Uint128::from(10000u128)).to_string(), "0.00000000000001");
		assert_eq!(Decimal(Uint128::from(100000u128)).to_string(), "0.0000000000001");
		assert_eq!(Decimal(Uint128::from(1000000u128)).to_string(), "0.000000000001");
		assert_eq!(Decimal(Uint128::from(10000000u128)).to_string(), "0.00000000001");
		assert_eq!(Decimal(Uint128::from(100000000u128)).to_string(), "0.0000000001");
		assert_eq!(Decimal(Uint128::from(1000000000u128)).to_string(), "0.000000001");
		assert_eq!(Decimal(Uint128::from(10000000000u128)).to_string(), "0.00000001");
		assert_eq!(Decimal(Uint128::from(100000000000u128)).to_string(), "0.0000001");
		assert_eq!(Decimal(Uint128::from(10000000000000u128)).to_string(), "0.00001");
		assert_eq!(Decimal(Uint128::from(100000000000000u128)).to_string(), "0.0001");
		assert_eq!(Decimal(Uint128::from(1000000000000000u128)).to_string(), "0.001");
		assert_eq!(Decimal(Uint128::from(10000000000000000u128)).to_string(), "0.01");
		assert_eq!(Decimal(Uint128::from(100000000000000000u128)).to_string(), "0.1");
	}

	#[test]
	fn decimal_iter_sum() {
		let items =
			vec![Decimal::zero(), Decimal(Uint128::from(2u128)), Decimal(Uint128::from(2u128))];
		assert_eq!(items.iter().sum::<Decimal>(), Decimal(Uint128::from(4u128)));
		assert_eq!(items.into_iter().sum::<Decimal>(), Decimal(Uint128::from(4u128)));
	...
	fn decimal_serialize() {
		...
		assert_eq!(serde_json::to_vec(&Decimal::percent(8)).unwrap(), br#""0.08""#);
		assert_eq!(serde_json::to_vec(&Decimal::percent(87)).unwrap(), br#""0.87""#);
		assert_eq!(serde_json::to_vec(&Decimal::percent(876)).unwrap(), br#""8.76""#);
		assert_eq!(serde_json::to_vec(&Decimal::percent(8765)).unwrap(), br#""87.65""#);
	}

	#[test]
	fn decimal_deserialize() {
		assert_eq!(serde_json::from_slice::<Decimal>(br#""0""#).unwrap(), Decimal::zero());
		assert_eq!(serde_json::from_slice::<Decimal>(br#""1""#).unwrap(), Decimal::one());
		assert_eq!(serde_json::from_slice::<Decimal>(br#""000""#).unwrap(), Decimal::zero());
		assert_eq!(serde_json::from_slice::<Decimal>(br#""001""#).unwrap(), Decimal::one());

		assert_eq!(serde_json::from_slice::<Decimal>(br#""0.08""#).unwrap(), Decimal::percent(8));
		assert_eq!(serde_json::from_slice::<Decimal>(br#""0.87""#).unwrap(), Decimal::percent(87));
		assert_eq!(serde_json::from_slice::<Decimal>(br#""8.76""#).unwrap(), Decimal::percent(876));
		assert_eq!(
	...
	fn decimal_rem_works() {
		// 4.02 % 1.11 = 0.69
		assert_eq!(Decimal::percent(402) % Decimal::percent(111), Decimal::percent(69));

		// 15.25 % 4 = 3.25
		assert_eq!(Decimal::percent(1525) % Decimal::percent(400), Decimal::percent(325));
	...
	fn decimal_checked_methods() {
		...
		assert!(matches!(Decimal::MAX.checked_add(Decimal::percent(1)), Err(OverflowError { .. })));
		...
		assert!(matches!(Decimal::MAX.checked_rem(Decimal::zero()), Err(DivideByZeroError { .. })));
	}
	...
	fn decimal_saturating_works() {
		...
		assert_eq!(Decimal::MAX.saturating_add(Decimal::percent(200)), Decimal::MAX);
		assert_eq!(
			Decimal::percent(200).saturating_sub(Decimal::percent(100)),
			Decimal::percent(100)
		);
		assert_eq!(Decimal::zero().saturating_sub(Decimal::percent(200)), Decimal::zero());
		assert_eq!(
			Decimal::percent(200).saturating_mul(Decimal::percent(50)),
			Decimal::percent(100)
		);
		assert_eq!(Decimal::MAX.saturating_mul(Decimal::percent(200)), Decimal::MAX);
		assert_eq!(Decimal::percent(400).saturating_pow(2u32), Decimal::percent(1600));
	...
	fn decimal_checked_ceil() {
		assert_eq!(Decimal::percent(199).checked_ceil(), Ok(Decimal::percent(200)));
		assert!(matches!(Decimal::MAX.checked_ceil(), Err(RoundUpOverflowError { .. })));
	}
