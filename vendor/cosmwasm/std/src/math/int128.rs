...
use super::{
	conversion::{forward_try_from, try_from_int_to_int},
...
impl Int128 {
	...
	pub fn full_mul(self, rhs: impl Into<Self>) -> Int256 {
		Int256::from(self).checked_mul(Int256::from(rhs.into())).unwrap()
	}
...
mod tests {
	...
	fn int128_wrapping_methods() {
		// wrapping_add
		assert_eq!(Int128::from(2u32).wrapping_add(Int128::from(2u32)), Int128::from(4u32)); // non-wrapping
		assert_eq!(Int128::MAX.wrapping_add(Int128::from(1u32)), Int128::MIN); // wrapping

		// wrapping_sub
		assert_eq!(Int128::from(7u32).wrapping_sub(Int128::from(5u32)), Int128::from(2u32)); // non-wrapping
		assert_eq!(Int128::MIN.wrapping_sub(Int128::from(1u32)), Int128::MAX); // wrapping

		// wrapping_mul
		assert_eq!(Int128::from(3u32).wrapping_mul(Int128::from(2u32)), Int128::from(6u32)); // non-wrapping
		assert_eq!(Int128::MAX.wrapping_mul(Int128::from(2u32)), Int128::from(-2i32)); // wrapping
	...
	fn int128_checked_multiply_ratio_works() {
		...
		// factor 3/2
		assert_eq!(base.checked_multiply_ratio(3i128, 2i128).unwrap(), Int128(750));
		assert_eq!(base.checked_multiply_ratio(333333i128, 222222i128).unwrap(), Int128(750));

		// factor 2/3 (integer devision always floors the result)
		assert_eq!(base.checked_multiply_ratio(2i128, 3i128).unwrap(), Int128(333));
		assert_eq!(base.checked_multiply_ratio(222222i128, 333333i128).unwrap(), Int128(333));

		// factor 5/6 (integer devision always floors the result)
		assert_eq!(base.checked_multiply_ratio(5i128, 6i128).unwrap(), Int128(416));
		assert_eq!(base.checked_multiply_ratio(100i128, 120i128).unwrap(), Int128(416));
	}
	...
	fn int128_methods() {
		// checked_*
		assert!(matches!(Int128::MAX.checked_add(Int128::from(1u32)), Err(OverflowError { .. })));
		assert_eq!(Int128::from(1u32).checked_add(Int128::from(1u32)), Ok(Int128::from(2u32)),);
		assert!(matches!(Int128::MIN.checked_sub(Int128::from(1u32)), Err(OverflowError { .. })));
		assert_eq!(Int128::from(2u32).checked_sub(Int128::from(1u32)), Ok(Int128::from(1u32)),);
		assert!(matches!(Int128::MAX.checked_mul(Int128::from(2u32)), Err(OverflowError { .. })));
		assert_eq!(Int128::from(2u32).checked_mul(Int128::from(2u32)), Ok(Int128::from(4u32)),);
		assert!(matches!(Int128::MAX.checked_pow(2u32), Err(OverflowError { .. })));
		assert_eq!(Int128::from(2u32).checked_pow(3u32), Ok(Int128::from(8u32)),);
		assert_eq!(Int128::MAX.checked_div(Int128::from(0u32)), Err(DivisionError::DivideByZero));
		assert_eq!(Int128::from(6u32).checked_div(Int128::from(2u32)), Ok(Int128::from(3u32)),);
		assert_eq!(
		...
		assert_eq!(Int128::from(-12i32).checked_div(Int128::from(10i32)), Ok(Int128::from(-1i32)),);
		assert_eq!(Int128::from(-2i32).checked_pow(3u32), Ok(Int128::from(-8i32)),);
		assert_eq!(Int128::from(-6i32).checked_mul(Int128::from(-7i32)), Ok(Int128::from(42i32)),);
		assert_eq!(Int128::from(-2i32).checked_add(Int128::from(3i32)), Ok(Int128::from(1i32)),);
		assert_eq!(
	...
	fn int128_implements_rem() {
		...
		assert_eq!(Int128::from(-12i32) % Int128::from(10i32), Int128::from(-2i32));
		assert_eq!(Int128::from(12i32) % Int128::from(-10i32), Int128::from(2i32));
		assert_eq!(Int128::from(-12i32) % Int128::from(-10i32), Int128::from(-2i32));
	...
	fn int128_shr() {
		...
		assert_eq!(x >> 1, Int128::from(0x2000_0000_0000_0000_0000_0000_0000_0000i128));
		assert_eq!(x >> 4, Int128::from(0x0400_0000_0000_0000_0000_0000_0000_0000i128));
		// right shift of MIN value by the maximum shift value should result in -1 (filled with 1s)
		assert_eq!(Int128::MIN >> (core::mem::size_of::<Int128>() as u32 * 8 - 1), -Int128::one());
	}
	...
	fn int128_shl() {
		...
		assert_eq!(x << 1, Int128::from(0x1000_0000_0000_0000_0000_0000_0000_0000i128));
		assert_eq!(x << 4, Int128::from(0x0800_0000_0000_0000_0000_0000_0000_0000i128 << 4));
		// left shift by by the maximum shift value should result in MIN
		assert_eq!(Int128::one() << (core::mem::size_of::<Int128>() as u32 * 8 - 1), Int128::MIN);
	}
