...
use crate::{
	...
	Decimal, Decimal256, Int256, SignedDecimal256,
	__internal::forward_ref_partial_eq,
};

use super::{Fraction, Int128};

/// A signed fixed-point decimal value with 18 fractional digits, i.e.
/// SignedDecimal(1_000_000_000_000_000_000) == 1.0
///
/// The greatest possible value that can be represented is 170141183460469231731.687303715884105727
/// (which is (2^127 - 1) / 10^18) and the smallest is -170141183460469231731.687303715884105728
/// (which is -2^127 / 10^18).
...
impl SignedDecimal {
	...
	/// This is equivalent to `SignedDecimal::from_atomics(value, 18)` but usable in a const
	/// context.
	///
	/// # Examples
	///
	...
	pub fn from_atomics(
		...
		Ok(match decimal_places.cmp(&(Self::DECIMAL_PLACES)) {
			Ordering::Less => {
				...
				Self(atomics.checked_mul(factor).map_err(|_| SignedDecimalRangeExceeded)?)
			},
	...
	pub fn checked_floor(&self) -> Result<Self, RoundDownOverflowError> {
		if self.is_negative() {
			...
			if truncated != self {
				truncated.checked_sub(SignedDecimal::one()).map_err(|_| RoundDownOverflowError)
			} else {
				Ok(truncated)
			}
		} else {
	...
	pub fn checked_ceil(&self) -> Result<Self, RoundUpOverflowError> {
		...
		if floor == self {
			...
			floor.checked_add(SignedDecimal::one()).map_err(|_| RoundUpOverflowError)
		}
	}
	...
	/// Multiplies one `SignedDecimal` by another, returning an `OverflowError` if an overflow
	/// occurred.
	...
	pub fn checked_rem(self, other: Self) -> Result<Self, DivideByZeroError> {
		self.0.checked_rem(other.0).map(Self).map_err(|_| DivideByZeroError)
	}
...
impl FromStr for SignedDecimal {
	...
	fn from_str(input: &str) -> Result<Self, Self::Err> {
		...
		if let Some(fractional_part) = parts_iter.next() {
			...
			let fractional_part = Int128::from(fractional).checked_mul(fractional_factor).unwrap();
...
mod tests {
	...
	fn signed_decimal_percent() {
		...
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(2u8));

		let value = SignedDecimal::percent(-50);
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(-2i8));
	}

	#[test]
	fn signed_decimal_permille() {
		let value = SignedDecimal::permille(125);
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(8u8));

		let value = SignedDecimal::permille(-125);
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(-8i8));
	}

	#[test]
	fn signed_decimal_bps() {
		let value = SignedDecimal::bps(125);
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(80u8));

		let value = SignedDecimal::bps(-125);
		assert_eq!(value.0, SignedDecimal::DECIMAL_FRACTIONAL / Int128::from(-80i8));
	}
	...
	fn signed_decimal_from_atomics_works() {
		...
		assert_eq!(SignedDecimal::from_atomics(1000000000000000000i128, 18).unwrap(), one);
		assert_eq!(SignedDecimal::from_atomics(10000000000000000000i128, 19).unwrap(), one);
		assert_eq!(SignedDecimal::from_atomics(100000000000000000000i128, 20).unwrap(), one);
		...
		assert_eq!(SignedDecimal::from_atomics(2000000000000000000i128, 18).unwrap(), two);
		assert_eq!(SignedDecimal::from_atomics(20000000000000000000i128, 19).unwrap(), two);
		assert_eq!(SignedDecimal::from_atomics(200000000000000000000i128, 20).unwrap(), two);

		assert_eq!(SignedDecimal::from_atomics(-1i128, 0).unwrap(), neg_one);
		assert_eq!(SignedDecimal::from_atomics(-10i128, 1).unwrap(), neg_one);
		assert_eq!(SignedDecimal::from_atomics(-100000000000000000000i128, 20).unwrap(), neg_one);
		...
		assert_eq!(SignedDecimal::from_atomics(max.atomics(), max.decimal_places()).unwrap(), max);

		// Can be used with min value
		let min = SignedDecimal::MIN;
		assert_eq!(SignedDecimal::from_atomics(min.atomics(), min.decimal_places()).unwrap(), min);
	...
	fn signed_decimal_from_ratio_works() {
		// 1.0
		assert_eq!(SignedDecimal::from_ratio(1i128, 1i128), SignedDecimal::one());
		assert_eq!(SignedDecimal::from_ratio(53i128, 53i128), SignedDecimal::one());
		assert_eq!(SignedDecimal::from_ratio(125i128, 125i128), SignedDecimal::one());

		// -1.0
		assert_eq!(SignedDecimal::from_ratio(-1i128, 1i128), SignedDecimal::negative_one());
		assert_eq!(SignedDecimal::from_ratio(-53i128, 53i128), SignedDecimal::negative_one());
		assert_eq!(SignedDecimal::from_ratio(125i128, -125i128), SignedDecimal::negative_one());

		// 1.5
		assert_eq!(SignedDecimal::from_ratio(3i128, 2i128), SignedDecimal::percent(150));
		assert_eq!(SignedDecimal::from_ratio(150i128, 100i128), SignedDecimal::percent(150));
		assert_eq!(SignedDecimal::from_ratio(333i128, 222i128), SignedDecimal::percent(150));

		// 0.125
		assert_eq!(SignedDecimal::from_ratio(1i64, 8i64), SignedDecimal::permille(125));
		assert_eq!(SignedDecimal::from_ratio(125i64, 1000i64), SignedDecimal::permille(125));
		...
		// large inputs
		assert_eq!(SignedDecimal::from_ratio(0i128, i128::MAX), SignedDecimal::zero());
		assert_eq!(SignedDecimal::from_ratio(i128::MAX, i128::MAX), SignedDecimal::one());
	...
	fn signed_decimal_implements_fraction() {
		...
		assert_eq!(fraction.numerator(), Int128::from(1_234_567_000_000_000_000_000i128));
		assert_eq!(fraction.denominator(), Int128::from(1_000_000_000_000_000_000i128));
	...
	fn signed_decimal_from_str_works() {
		// Integers
		assert_eq!(SignedDecimal::from_str("0").unwrap(), SignedDecimal::percent(0));
		assert_eq!(SignedDecimal::from_str("1").unwrap(), SignedDecimal::percent(100));
		assert_eq!(SignedDecimal::from_str("5").unwrap(), SignedDecimal::percent(500));
		assert_eq!(SignedDecimal::from_str("42").unwrap(), SignedDecimal::percent(4200));
		assert_eq!(SignedDecimal::from_str("000").unwrap(), SignedDecimal::percent(0));
		assert_eq!(SignedDecimal::from_str("001").unwrap(), SignedDecimal::percent(100));
		assert_eq!(SignedDecimal::from_str("005").unwrap(), SignedDecimal::percent(500));
		assert_eq!(SignedDecimal::from_str("0042").unwrap(), SignedDecimal::percent(4200));

		// Positive decimals
		assert_eq!(SignedDecimal::from_str("1.0").unwrap(), SignedDecimal::percent(100));
		assert_eq!(SignedDecimal::from_str("1.5").unwrap(), SignedDecimal::percent(150));
		assert_eq!(SignedDecimal::from_str("0.5").unwrap(), SignedDecimal::percent(50));
		assert_eq!(SignedDecimal::from_str("0.123").unwrap(), SignedDecimal::permille(123));

		assert_eq!(SignedDecimal::from_str("40.00").unwrap(), SignedDecimal::percent(4000));
		assert_eq!(SignedDecimal::from_str("04.00").unwrap(), SignedDecimal::percent(400));
		assert_eq!(SignedDecimal::from_str("00.40").unwrap(), SignedDecimal::percent(40));
		assert_eq!(SignedDecimal::from_str("00.04").unwrap(), SignedDecimal::percent(4));
		// Negative decimals
		assert_eq!(SignedDecimal::from_str("-00.04").unwrap(), SignedDecimal::percent(-4));
		assert_eq!(SignedDecimal::from_str("-00.40").unwrap(), SignedDecimal::percent(-40));
		assert_eq!(SignedDecimal::from_str("-04.00").unwrap(), SignedDecimal::percent(-400));
		...
		assert_eq!(SignedDecimal::from_str("-1").unwrap(), SignedDecimal::negative_one());
	}
	...
	fn signed_decimal_from_str_errors_for_more_than_18_fractional_digits() {
		...
		assert_eq!(SignedDecimal::from_str("7.1234567890123456789").unwrap_err(), expected_err);
		// No special rules for trailing zeros. This could be changed but adds gas cost for the
		// happy path.
		assert_eq!(SignedDecimal::from_str("7.1230000000000000000").unwrap_err(), expected_err);
	}
	...
	fn signed_decimal_from_str_errors_for_invalid_number_of_dots() {
		...
		assert_eq!(SignedDecimal::from_str("1.2.3.4").unwrap_err(), expected_err);
	}
	...
	fn signed_decimal_from_str_errors_for_more_than_max_value() {
		...
		// Integer
		assert_eq!(SignedDecimal::from_str("170141183460469231732").unwrap_err(), expected_err);
		assert_eq!(SignedDecimal::from_str("-170141183460469231732").unwrap_err(), expected_err);

		// SignedDecimal
		assert_eq!(SignedDecimal::from_str("170141183460469231732.0").unwrap_err(), expected_err);
		assert_eq!(
	...
	fn signed_decimal_conversions_work() {
		...
		assert_eq!(SignedDecimal::try_from(SignedDecimal256::one()).unwrap(), SignedDecimal::one());
		assert_eq!(
		...
		assert_eq!(SignedDecimal::try_from(Decimal::MAX).unwrap_err(), SignedDecimalRangeExceeded);
		let max = Decimal::raw(SignedDecimal::MAX.atomics().i128() as u128);
		let too_big = max + Decimal::raw(1);
		assert_eq!(SignedDecimal::try_from(too_big).unwrap_err(), SignedDecimalRangeExceeded);
		assert_eq!(SignedDecimal::try_from(Decimal::zero()).unwrap(), SignedDecimal::zero());
	...
	fn signed_decimal_inv_works() {
		...
		// d == -1
		assert_eq!(SignedDecimal::negative_one().inv(), Some(SignedDecimal::negative_one()));
	...
	fn signed_decimal_add_works() {
		...
		assert_eq!(SignedDecimal::percent(5) + SignedDecimal::zero(), SignedDecimal::percent(5));
		assert_eq!(SignedDecimal::zero() + SignedDecimal::zero(), SignedDecimal::zero());
		// negative numbers
		assert_eq!(
			SignedDecimal::percent(-5) + SignedDecimal::percent(-4),
			SignedDecimal::percent(-9)
	...
	fn signed_decimal_sub_works() {
		...
		assert_eq!(SignedDecimal::percent(16) - SignedDecimal::zero(), SignedDecimal::percent(16));
		assert_eq!(SignedDecimal::percent(16) - SignedDecimal::percent(16), SignedDecimal::zero());
		assert_eq!(SignedDecimal::zero() - SignedDecimal::zero(), SignedDecimal::zero());
	...
	fn signed_decimal_implements_mul() {
		...
		assert_eq!(one * SignedDecimal::percent(100), SignedDecimal::percent(100));
		assert_eq!(one * SignedDecimal::percent(1000), SignedDecimal::percent(1000));
		...
		assert_eq!(SignedDecimal::percent(100) * one, SignedDecimal::percent(100));
		assert_eq!(SignedDecimal::percent(1000) * one, SignedDecimal::percent(1000));
		...
		assert_eq!(two * SignedDecimal::percent(100), SignedDecimal::percent(200));
		assert_eq!(two * SignedDecimal::percent(1000), SignedDecimal::percent(2000));
		...
		assert_eq!(SignedDecimal::percent(100) * two, SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(1000) * two, SignedDecimal::percent(2000));
		assert_eq!(SignedDecimal::percent(-1) * two, SignedDecimal::percent(-2));
		assert_eq!(two * SignedDecimal::new(Int128::MIN / Int128::new(2)), SignedDecimal::MIN);
		...
		assert_eq!(half * SignedDecimal::percent(100), SignedDecimal::percent(50));
		assert_eq!(half * SignedDecimal::percent(1000), SignedDecimal::percent(500));
		...
		assert_eq!(SignedDecimal::percent(100) * half, SignedDecimal::percent(50));
		assert_eq!(SignedDecimal::percent(1000) * half, SignedDecimal::percent(500));
		...
		// Move right
		let max = SignedDecimal::MAX;
		assert_eq!(max * dec("1.0"), dec("170141183460469231731.687303715884105727"));
		assert_eq!(max * dec("0.1"), dec("17014118346046923173.168730371588410572"));
		assert_eq!(max * dec("0.01"), dec("1701411834604692317.316873037158841057"));
		assert_eq!(max * dec("0.001"), dec("170141183460469231.731687303715884105"));
		assert_eq!(max * dec("0.000001"), dec("170141183460469.231731687303715884"));
		assert_eq!(max * dec("0.000000001"), dec("170141183460.469231731687303715"));
		assert_eq!(max * dec("0.000000000001"), dec("170141183.460469231731687303"));
		assert_eq!(max * dec("0.000000000000001"), dec("170141.183460469231731687"));
		assert_eq!(max * dec("0.000000000000000001"), dec("170.141183460469231731"));
	...
	fn signed_decimal_checked_mul() {
		let test_data = [
			...
			(SignedDecimal::MAX / Int128::new(2), SignedDecimal::percent(200)),
	...
	fn signed_decimal_implements_div() {
		...
		// 1/x and x/1
		assert_eq!(one / SignedDecimal::percent(1), SignedDecimal::percent(10_000));
		assert_eq!(one / SignedDecimal::percent(10), SignedDecimal::percent(1_000));
		assert_eq!(one / SignedDecimal::percent(100), SignedDecimal::percent(100));
		assert_eq!(one / SignedDecimal::percent(1000), SignedDecimal::percent(10));
		assert_eq!(SignedDecimal::percent(0) / one, SignedDecimal::percent(0));
		assert_eq!(SignedDecimal::percent(1) / one, SignedDecimal::percent(1));
		assert_eq!(SignedDecimal::percent(10) / one, SignedDecimal::percent(10));
		assert_eq!(SignedDecimal::percent(100) / one, SignedDecimal::percent(100));
		...
		// double
		assert_eq!(two / SignedDecimal::percent(1), SignedDecimal::percent(20_000));
		assert_eq!(two / SignedDecimal::percent(10), SignedDecimal::percent(2_000));
		assert_eq!(two / SignedDecimal::percent(100), SignedDecimal::percent(200));
		assert_eq!(two / SignedDecimal::percent(1000), SignedDecimal::percent(20));
		assert_eq!(SignedDecimal::percent(0) / two, SignedDecimal::percent(0));
		assert_eq!(SignedDecimal::percent(1) / two, dec("0.005"));
		assert_eq!(SignedDecimal::percent(10) / two, SignedDecimal::percent(5));
		assert_eq!(SignedDecimal::percent(100) / two, SignedDecimal::percent(50));
		assert_eq!(SignedDecimal::percent(1000) / two, SignedDecimal::percent(500));
		assert_eq!(two / SignedDecimal::percent(-1), SignedDecimal::percent(-20_000));
		assert_eq!(SignedDecimal::percent(-10000) / two, SignedDecimal::percent(-5000));

		// half
		assert_eq!(half / SignedDecimal::percent(1), SignedDecimal::percent(5_000));
		assert_eq!(half / SignedDecimal::percent(10), SignedDecimal::percent(500));
		assert_eq!(half / SignedDecimal::percent(100), SignedDecimal::percent(50));
		assert_eq!(half / SignedDecimal::percent(1000), SignedDecimal::percent(5));
		assert_eq!(SignedDecimal::percent(0) / half, SignedDecimal::percent(0));
		assert_eq!(SignedDecimal::percent(1) / half, SignedDecimal::percent(2));
		assert_eq!(SignedDecimal::percent(10) / half, SignedDecimal::percent(20));
		assert_eq!(SignedDecimal::percent(100) / half, SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(1000) / half, SignedDecimal::percent(2000));
	...
	fn signed_decimal_checked_pow() {
		for exp in 0..10 {
			assert_eq!(SignedDecimal::one().checked_pow(exp).unwrap(), SignedDecimal::one());
		}
		...
		assert_eq!(SignedDecimal::zero().checked_pow(0).unwrap(), SignedDecimal::one());

		for exp in 1..10 {
			assert_eq!(SignedDecimal::zero().checked_pow(exp).unwrap(), SignedDecimal::zero());
		}
		...
		for exp in 1..10 {
			assert_eq!(
				...
				if exp % 2 == 0 { SignedDecimal::one() } else { SignedDecimal::negative_one() }
			)
		}

		for num in
			&[SignedDecimal::percent(50), SignedDecimal::percent(99), SignedDecimal::percent(200)]
		{
			assert_eq!(num.checked_pow(0).unwrap(), SignedDecimal::one())
		}

		assert_eq!(SignedDecimal::percent(20).checked_pow(2).unwrap(), SignedDecimal::percent(4));

		assert_eq!(SignedDecimal::percent(20).checked_pow(3).unwrap(), SignedDecimal::permille(8));
	...
	fn signed_decimal_to_string() {
		...
		assert_eq!(SignedDecimal(Int128::from(1i128)).to_string(), "0.000000000000000001");
		assert_eq!(SignedDecimal(Int128::from(10i128)).to_string(), "0.00000000000000001");
		assert_eq!(SignedDecimal(Int128::from(100i128)).to_string(), "0.0000000000000001");
		assert_eq!(SignedDecimal(Int128::from(1000i128)).to_string(), "0.000000000000001");
		assert_eq!(SignedDecimal(Int128::from(10000i128)).to_string(), "0.00000000000001");
		assert_eq!(SignedDecimal(Int128::from(100000i128)).to_string(), "0.0000000000001");
		assert_eq!(SignedDecimal(Int128::from(1000000i128)).to_string(), "0.000000000001");
		assert_eq!(SignedDecimal(Int128::from(10000000i128)).to_string(), "0.00000000001");
		assert_eq!(SignedDecimal(Int128::from(100000000i128)).to_string(), "0.0000000001");
		assert_eq!(SignedDecimal(Int128::from(1000000000i128)).to_string(), "0.000000001");
		assert_eq!(SignedDecimal(Int128::from(10000000000i128)).to_string(), "0.00000001");
		assert_eq!(SignedDecimal(Int128::from(100000000000i128)).to_string(), "0.0000001");
		assert_eq!(SignedDecimal(Int128::from(10000000000000i128)).to_string(), "0.00001");
		assert_eq!(SignedDecimal(Int128::from(100000000000000i128)).to_string(), "0.0001");
		assert_eq!(SignedDecimal(Int128::from(1000000000000000i128)).to_string(), "0.001");
		assert_eq!(SignedDecimal(Int128::from(10000000000000000i128)).to_string(), "0.01");
		assert_eq!(SignedDecimal(Int128::from(100000000000000000i128)).to_string(), "0.1");
		assert_eq!(SignedDecimal(Int128::from(-1i128)).to_string(), "-0.000000000000000001");
		assert_eq!(SignedDecimal(Int128::from(-100000000000000i128)).to_string(), "-0.0001");
		assert_eq!(SignedDecimal(Int128::from(-100000000000000000i128)).to_string(), "-0.1");
	}
	...
	fn signed_decimal_iter_sum() {
		...
		assert_eq!(items.iter().sum::<SignedDecimal>(), SignedDecimal(Int128::from(2i128)));
		assert_eq!(items.into_iter().sum::<SignedDecimal>(), SignedDecimal(Int128::from(2i128)));
	...
	fn signed_decimal_serialize() {
		assert_eq!(serde_json::to_vec(&SignedDecimal::zero()).unwrap(), br#""0""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::one()).unwrap(), br#""1""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::percent(8)).unwrap(), br#""0.08""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::percent(87)).unwrap(), br#""0.87""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::percent(876)).unwrap(), br#""8.76""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::percent(8765)).unwrap(), br#""87.65""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::percent(-87654)).unwrap(), br#""-876.54""#);
		assert_eq!(serde_json::to_vec(&SignedDecimal::negative_one()).unwrap(), br#""-1""#);
		assert_eq!(serde_json::to_vec(&-SignedDecimal::percent(8)).unwrap(), br#""-0.08""#);
	}
	...
	fn signed_decimal_checked_methods() {
		// checked add
		assert_eq!(
			SignedDecimal::percent(402).checked_add(SignedDecimal::percent(111)).unwrap(),
		...
		// checked sub
		assert_eq!(
			SignedDecimal::percent(1111).checked_sub(SignedDecimal::percent(111)).unwrap(),
			SignedDecimal::percent(1000)
		);
		assert_eq!(
			SignedDecimal::zero().checked_sub(SignedDecimal::percent(1)).unwrap(),
			SignedDecimal::percent(-1)
		);
		assert!(matches!(
		...
		// checked div
		assert_eq!(
			SignedDecimal::percent(30).checked_div(SignedDecimal::percent(200)).unwrap(),
			SignedDecimal::percent(15)
		);
		assert_eq!(
			SignedDecimal::percent(88).checked_div(SignedDecimal::percent(20)).unwrap(),
		...
		// checked rem
		assert_eq!(
			SignedDecimal::percent(402).checked_rem(SignedDecimal::percent(111)).unwrap(),
			SignedDecimal::percent(69)
		);
		assert_eq!(
			SignedDecimal::percent(1525).checked_rem(SignedDecimal::percent(400)).unwrap(),
			SignedDecimal::percent(325)
		);
		assert_eq!(
			SignedDecimal::percent(-1525).checked_rem(SignedDecimal::percent(400)).unwrap(),
	...
	fn signed_decimal_pow_works() {
		assert_eq!(SignedDecimal::percent(200).pow(2), SignedDecimal::percent(400));
		assert_eq!(SignedDecimal::percent(-200).pow(2), SignedDecimal::percent(400));
		assert_eq!(SignedDecimal::percent(-200).pow(3), SignedDecimal::percent(-800));
		assert_eq!(SignedDecimal::percent(200).pow(10), SignedDecimal::percent(102400));
	}
	...
	fn signed_decimal_saturating_works() {
		...
		assert_eq!(SignedDecimal::percent(400).saturating_pow(2u32), SignedDecimal::percent(1600));
	...
	fn signed_decimal_rounding() {
		...
		assert_eq!(SignedDecimal::percent(200).floor(), SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(99).floor(), SignedDecimal::zero());
		assert_eq!(SignedDecimal(Int128::from(1i128)).floor(), SignedDecimal::zero());
		assert_eq!(SignedDecimal(Int128::from(-1i128)).floor(), SignedDecimal::negative_one());
		assert_eq!(SignedDecimal::permille(-1234).floor(), SignedDecimal::percent(-200));

		assert_eq!(SignedDecimal::one().ceil(), SignedDecimal::one());
		assert_eq!(SignedDecimal::percent(150).ceil(), SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(199).ceil(), SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(99).ceil(), SignedDecimal::one());
		assert_eq!(SignedDecimal(Int128::from(1i128)).ceil(), SignedDecimal::one());
		assert_eq!(SignedDecimal(Int128::from(-1i128)).ceil(), SignedDecimal::zero());
		assert_eq!(SignedDecimal::permille(-1234).ceil(), SignedDecimal::negative_one());
		...
		assert_eq!(SignedDecimal::percent(200).trunc(), SignedDecimal::percent(200));
		assert_eq!(SignedDecimal::percent(99).trunc(), SignedDecimal::zero());
		assert_eq!(SignedDecimal(Int128::from(1i128)).trunc(), SignedDecimal::zero());
		assert_eq!(SignedDecimal(Int128::from(-1i128)).trunc(), SignedDecimal::zero());
		assert_eq!(SignedDecimal::permille(-1234).trunc(), SignedDecimal::negative_one());
	}
	...
	fn signed_decimal_checked_ceil() {
		assert_eq!(SignedDecimal::percent(199).checked_ceil(), Ok(SignedDecimal::percent(200)));
		assert_eq!(SignedDecimal::MAX.checked_ceil(), Err(RoundUpOverflowError));
	}

	#[test]
	fn signed_decimal_checked_floor() {
		assert_eq!(SignedDecimal::percent(199).checked_floor(), Ok(SignedDecimal::one()));
		assert_eq!(SignedDecimal::percent(-199).checked_floor(), Ok(SignedDecimal::percent(-200)));
		assert_eq!(SignedDecimal::MIN.checked_floor(), Err(RoundDownOverflowError));
		assert_eq!(
	...
	fn signed_decimal_can_be_serialized_and_deserialized() {
		...
		assert_eq!(
			...
			serde_json::from_str::<SignedDecimal>("123").err().unwrap().to_string()
		);
