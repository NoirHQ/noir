...
use crate::{
	...
	impl_mul_fraction, Fraction, Int128, Int256, Int512, Int64, Uint128,
};
...
impl Uint64 {
	...
	pub fn full_mul(self, rhs: impl Into<Self>) -> Uint128 {
		Uint128::from(self).checked_mul(Uint128::from(rhs.into())).unwrap()
	}
...
impl Mul<Uint64> for Uint64 {
	...
	fn mul(self, rhs: Self) -> Self::Output {
		Self(self.u64().checked_mul(rhs.u64()).expect("attempt to multiply with overflow"))
	}
}
...
impl Shl<u32> for Uint64 {
	...
	fn shl(self, rhs: u32) -> Self::Output {
		Self(self.u64().checked_shl(rhs).expect("attempt to shift left with overflow"))
	}
}
...
mod tests {
	...
	fn uint64_to_be_bytes_works() {
		...
		assert_eq!(Uint64::MAX.to_be_bytes(), [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]);
	...
	fn uint64_to_le_bytes_works() {
		...
		assert_eq!(Uint64::MAX.to_le_bytes(), [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]);
	...
	fn uint64_shr_works() {
		...
		let shifted = Uint64::new(u64::from_be_bytes([0u8, 0u8, 0u8, 0u8, 0u8, 128u8, 1u8, 0u8]));
	...
	#[test]
	fn uint64_shl_works() {
		let original = Uint64::new(u64::from_be_bytes([64u8, 128u8, 1u8, 0u8, 0u8, 0u8, 0u8, 0u8]));
	...
	fn uint64_methods() {
		// checked_*
		assert!(matches!(Uint64::MAX.checked_add(Uint64(1)), Err(OverflowError { .. })));
		assert!(matches!(Uint64(1).checked_add(Uint64(1)), Ok(Uint64(2))));
		assert!(matches!(Uint64(0).checked_sub(Uint64(1)), Err(OverflowError { .. })));
		assert!(matches!(Uint64(2).checked_sub(Uint64(1)), Ok(Uint64(1))));
		assert!(matches!(Uint64::MAX.checked_mul(Uint64(2)), Err(OverflowError { .. })));
		assert!(matches!(Uint64(2).checked_mul(Uint64(2)), Ok(Uint64(4))));
		assert!(matches!(Uint64::MAX.checked_pow(2u32), Err(OverflowError { .. })));
		assert!(matches!(Uint64(2).checked_pow(3), Ok(Uint64(8))));
		assert!(matches!(Uint64::MAX.checked_div(Uint64(0)), Err(DivideByZeroError { .. })));
		assert!(matches!(Uint64(6).checked_div(Uint64(2)), Ok(Uint64(3))));
		assert!(matches!(Uint64::MAX.checked_div_euclid(Uint64(0)), Err(DivideByZeroError { .. })));
		assert!(matches!(Uint64(6).checked_div_euclid(Uint64(2)), Ok(Uint64(3)),));
		assert!(matches!(Uint64::MAX.checked_rem(Uint64(0)), Err(DivideByZeroError { .. })));
	...
	fn uint64_wrapping_methods() {
		...
		assert_eq!(Uint64::MAX.wrapping_mul(Uint64(2)), Uint64::MAX - Uint64::one()); // wrapping
	...
	#[allow(clippy::op_ref)]
	fn uint64_rem_works() {
		assert_eq!(Uint64::from(12u32) % Uint64::from(10u32), Uint64::from(2u32));
