	...
	String: From<I>,
{
	...
	// 42 should work
	assert_eq!(O::try_from(I::from(42u32)), Ok(O::from(42u32)), "42 should fit")
}
	...
	<I as core::convert::TryFrom<O>>::Error: core::fmt::Debug,
{
	if core::mem::size_of::<I>() <= core::mem::size_of::<O>() {
		...
		let v = I::MAX;
		assert_eq!(O::try_from(v), Ok(O::try_from(v).unwrap()), "input::MAX value should fit");
	} else {
		...
		let max = I::try_from(O::MAX).unwrap();
		assert_eq!(O::try_from(max), Ok(O::try_from(max).unwrap()), "output::MAX value should fit");
	...
	// 42 should work
	assert_eq!(O::try_from(I::from(42i32)), Ok(O::from(42u32)), "42 should fit")
}
...
mod tests {
	...
	fn shrink_be_int_works() {
		...
		for i in i32s {
			assert_eq!(shrink_be_int(i.to_be_bytes()), Some((i as i16).to_be_bytes()));
			assert_eq!(shrink_be_int(i.to_be_bytes()), Some((i as i8).to_be_bytes()));
		}
		// these should be too big to fit into an i16 or i8
		let oob =
			[i32::MIN, i32::MIN + 10, i32::MIN + 1234, i32::MAX - 1234, i32::MAX - 10, i32::MAX];
		for i in oob {
			// 32 -> 16 bit
		...
		for i in i16::MIN..=i16::MAX {
			...
			if i == cast {
				...
				assert_eq!(shrink_be_int::<2, 1>(i.to_be_bytes()), Some((i as i8).to_be_bytes()));
			} else {
