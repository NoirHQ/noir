...
use crate::{Binary, HexBinary, __internal::forward_ref_partial_eq};
...
/// 4. Deserialize from JSON. This must only be done from JSON that was validated before such as a
///    contract's state. `Addr` must not be used in messages sent by the user because this would
///    result in unvalidated instances.
///
...
mod tests {
	...
	fn canonical_addr_implements_from_and_to_vector() {
		...
		assert_eq!((addr.0).as_slice().as_ptr(), original_ptr, "must not be copied");
	...
	fn instantiate2_address_impl_works_for_cosmjs_test_vectors() {
		...
		for Row { input, intermediate, out: _ } in read_tests() {
