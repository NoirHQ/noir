...
impl MemoSource for WithCallbacks {
	fn into_memo(self) -> Option<String> {
		Some(
			to_json_string(&IbcCallbackRequest::both(self.src_callback, self.dst_callback))
				.unwrap(),
		)
	}
}
...
impl TransferMsgBuilder<WithSrcCallback> {
	...
	pub fn with_dst_callback(
		...
		TransferMsgBuilder {
			...
			amount: self.amount,
			timeout: self.timeout,
			memo: WithCallbacks { src_callback: self.memo.src_callback, dst_callback },
		}
	}
}
...
impl TransferMsgBuilder<WithDstCallback> {
	...
	pub fn with_src_callback(
		...
		TransferMsgBuilder {
			...
			amount: self.amount,
			timeout: self.timeout,
			memo: WithCallbacks { src_callback, dst_callback: self.memo.dst_callback },
		}
	}
}
...
mod tests {
	...
	fn test_transfer_msg_builder() {
		let src_callback =
			IbcSrcCallback { address: Addr::unchecked("src"), gas_limit: Some(Uint64::new(12345)) };
		let dst_callback = IbcDstCallback { address: "dst".to_string(), gas_limit: None };
		...
		let with_src_callback_builder =
			empty_memo_builder.clone().with_src_callback(src_callback.clone());
		let with_src_callback = with_src_callback_builder.clone().build();
		let with_dst_callback_builder =
			empty_memo_builder.clone().with_dst_callback(dst_callback.clone());
		let with_dst_callback = with_dst_callback_builder.clone().build();

		let with_both_callbacks1 =
			with_src_callback_builder.with_dst_callback(dst_callback.clone()).build();

		let with_both_callbacks2 =
			with_dst_callback_builder.with_src_callback(src_callback.clone()).build();
