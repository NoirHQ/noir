...
impl IbcCallbackRequest {
	...
	/// Use this if you want to execute callbacks on the source chain, but not the destination
	/// chain.
	pub fn source(src_callback: IbcSrcCallback) -> Self {
		IbcCallbackRequest { src_callback: Some(src_callback), dest_callback: None }
	}

	/// Use this if you want to execute callbacks on the destination chain, but not the source
	/// chain.
...
/// IBC packet through some other message (i.e. not through [`crate::IbcMsg::SendPacket`]) and needs
/// to know whether or not the packet was successfully received on the other chain.
/// A prominent example is the [`crate::IbcMsg::Transfer`] message. Without callbacks, you cannot
/// know whether the transfer was successful or not.
///
...
/// - You have to add serialized [`IbcCallbackRequest`] to a specific field of the message. For
///   `IbcMsg::Transfer`, this is the `memo` field and it needs to be json-encoded.
...
/// A prominent example is the [`crate::IbcMsg::Transfer`] message. Without callbacks, you cannot
/// know that someone sent you IBC coins.
///
...
///
/// Note that there are some prerequisites that need to be fulfilled to receive destination
/// callbacks:
...
mod tests {
	...
	fn ibc_callback_data_serialization() {
		let mut data = IbcCallbackRequest::both(
			...
			IbcDstCallback { address: "dst_address".to_string(), gas_limit: Some(1234u64.into()) },
		);
