...
impl<T> Default for Response<T> {
	fn default() -> Self {
		Response { messages: vec![], attributes: vec![], events: vec![], data: None }
	}
}
...
impl<T> Response<T> {
	...
	///
	/// For working with optional values or optional attributes, see
	...
	/// ```
	///
	/// Adding an optional value as an optional attribute by turning it into a list of 0 or 1
	/// elements:
	///
	/// ```
...
mod tests {
	...
	use crate::{
		coins, from_json,
		results::submessages::{ReplyOn, UNUSED_MSG_ID},
		to_json_vec, ContractResult,
	};
	...
	fn response_add_attributes_works() {
		...
		assert_eq!(
			res.attributes[0],
			Attribute { key: "test".to_string(), value: "ing".to_string() }
		);

		let attrs = vec![("action", "reaction"), ("answer", "42"), ("another", "attribute")];
		let res: Response = Response::new().add_attributes(attrs.clone());
		assert_eq!(res.attributes, attrs);
	...
	fn can_serialize_and_deserialize_init_response() {
		let original = Response {
			...
			attributes: vec![Attribute { key: "action".to_string(), value: "release".to_string() }],
			events: vec![],
