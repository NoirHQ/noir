...
use crate::{
	import_helpers::{from_high_half, from_low_half},
	memory::{Owned, Region},
	results::SystemResult,
	sections::encode_sections,
	serde::from_json,
	traits::{Api, Querier, QuerierResult, Storage},
	Addr, CanonicalAddr, RecoverPubkeyError, StdError, StdResult, SystemError, VerificationError,
};
#[cfg(feature = "iterator")]
use crate::{
...
/// An upper bound for typical canonical address lengths (e.g. 20 in Cosmos SDK/Ethereum or 32 in
/// Nano/Substrate)
const CANONICAL_ADDRESS_BUFFER_LENGTH: usize = 64;
/// An upper bound for typical human readable address formats (e.g. 42 for Ethereum hex addresses or
/// 90 for bech32)
...
/// This cannot be cloned as it would not copy any data. If you need to clone this, it indicates a
/// flaw in your logic.
...
impl Storage for ExternalStorage {
	...
	fn range_keys<'a>(
		...
		let iterator_id = create_iter(start, end, order);
		let iter = ExternalPartialIterator { iterator_id, partial_type: PartialType::Keys };
		Box::new(iter)
	}
	...
	fn range_values<'a>(
		...
		let iterator_id = create_iter(start, end, order);
		let iter = ExternalPartialIterator { iterator_id, partial_type: PartialType::Values };
		Box::new(iter)
	}
}
...
impl Api for ExternalApi {
	fn addr_validate(&self, input: &str) -> StdResult<Addr> {
		...
		if result != 0 {
			...
			return Err(StdError::generic_err(format!("addr_validate errored: {}", error)));
		}
	...
	fn addr_canonicalize(&self, input: &str) -> StdResult<CanonicalAddr> {
		...
		if result != 0 {
			let error =
				unsafe { consume_string_region_written_by_vm(result as *mut Region<Owned>) };
			return Err(StdError::generic_err(format!("addr_canonicalize errored: {}", error)));
		}
	...
	fn addr_humanize(&self, canonical: &CanonicalAddr) -> StdResult<Addr> {
		...
		if result != 0 {
			let error =
				unsafe { consume_string_region_written_by_vm(result as *mut Region<Owned>) };
			return Err(StdError::generic_err(format!("addr_humanize errored: {}", error)));
		}
	...
	fn bls12_381_aggregate_g1(&self, g1s: &[u8]) -> Result<[u8; 48], VerificationError> {
		...
		match result {
			...
			16 => Err(VerificationError::Aggregation { source: AggregationError::Empty }),
			17 => Err(VerificationError::Aggregation { source: AggregationError::NotMultiple }),
	...
	fn bls12_381_aggregate_g2(&self, g2s: &[u8]) -> Result<[u8; 96], VerificationError> {
		...
		match result {
			0 => Ok(point),
			8 => Err(VerificationError::InvalidPoint),
			14 => Err(VerificationError::Aggregation { source: AggregationError::Empty }),
			15 => Err(VerificationError::Aggregation { source: AggregationError::NotMultiple }),
			error_code => Err(VerificationError::unknown_err(error_code)),
		}
	}
	...
	fn debug(&self, message: &str) {
		// keep the boxes in scope, so we free it at the end (don't cast to pointers same line as
		// Region::from_slice)
