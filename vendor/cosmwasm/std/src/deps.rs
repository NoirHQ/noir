...
impl<S: Storage, A: Api, Q: Querier, C: CustomQuery> OwnedDeps<S, A, Q, C> {
	pub fn as_ref(&'_ self) -> Deps<'_, C> {
		Deps { storage: &self.storage, api: &self.api, querier: QuerierWrapper::new(&self.querier) }
	}
...
impl<'a, C: CustomQuery> DepsMut<'a, C> {
	pub fn as_ref(&'_ self) -> Deps<'_, C> {
		Deps { storage: self.storage, api: self.api, querier: self.querier }
	}

	pub fn branch(&'_ mut self) -> DepsMut<'_, C> {
		DepsMut { storage: self.storage, api: self.api, querier: self.querier }
	}

	/// This allows to convert any `DepsMut` into one generic over `Empty` custom
	/// query type.
	pub fn into_empty(self) -> DepsMut<'a, Empty> {
		DepsMut { storage: self.storage, api: self.api, querier: self.querier.into_empty() }
	}
}
...
impl<'a, C: CustomQuery> Deps<'a, C> {
	...
	pub fn into_empty(self) -> Deps<'a, Empty> {
		Deps { storage: self.storage, api: self.api, querier: self.querier.into_empty() }
	}
}
