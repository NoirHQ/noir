...
use crate::{prelude::*, Addr, Binary, Checksum};
...
mod tests {
	...
	fn wasm_query_contract_info_serialization() {
		let query = WasmQuery::ContractInfo { contract_addr: "aabbccdd456".into() };
	...
	fn wasm_query_code_info_serialization() {
		...
		let json = to_json_binary(&query).unwrap();
		assert_eq!(String::from_utf8_lossy(&json), r#"{"code_info":{"code_id":70}}"#,);
	}
