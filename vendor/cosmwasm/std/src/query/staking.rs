...
use crate::{prelude::*, Addr, Coin, Decimal};
...
pub enum StakingQuery {
	/// Returns the denomination that can be bonded (if there are multiple native tokens on the
	/// chain)
	BondedDenom {},
	...
	Delegation { delegator: String, validator: String },
...
impl From<FullDelegation> for Delegation {
	fn from(full: FullDelegation) -> Self {
		Delegation { delegator: full.delegator, validator: full.validator, amount: full.amount }
	}
}
...
/// FullDelegation is all the info on the delegation, some (like accumulated_reward and
/// can_redelegate) is expensive to query.
///
/// Instances are created in the querier.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema)]
#[non_exhaustive]
...
impl FullDelegation {
	...
	pub fn create(
		...
		Self { delegator, validator, amount, can_redelegate, accumulated_rewards }
	}
}
...
impl Validator {
	...
	pub fn create(
		...
		Self { address, commission, max_commission, max_change_rate }
	}
}
