...
mod tests {
	...
	fn system_error_no_such_contract_serialization() {
		let err = SystemError::NoSuchContract { addr: "gibtsnicht".to_string() };

		// ser
		let json = serde_json::to_vec(&err).unwrap();
		assert_eq!(String::from_utf8_lossy(&json), r#"{"no_such_contract":{"addr":"gibtsnicht"}}"#,);

		// de
		let err: SystemError =
			serde_json::from_slice(br#"{"no_such_contract":{"addr":"nada"}}"#).unwrap();
		assert_eq!(err, SystemError::NoSuchContract { addr: "nada".to_string() });
	}
	...
	fn system_error_no_such_code_serialization() {
		...
		// ser
		let json = serde_json::to_vec(&err).unwrap();
		assert_eq!(String::from_utf8_lossy(&json), r#"{"no_such_code":{"code_id":13}}"#,);
