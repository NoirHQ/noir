...
pub enum StdError {
	...
	VerificationErr { source: VerificationError, backtrace: BT },
	#[error("Recover pubkey error: {source}")]
	RecoverPubkeyErr { source: RecoverPubkeyError, backtrace: BT },
	...
	InvalidDataSize { expected: u64, actual: u64, backtrace: BT },
	#[error("Invalid hex string: {msg}")]
	InvalidHex { msg: String, backtrace: BT },
	/// Whenever UTF-8 bytes cannot be decoded into a unicode string, e.g. in String::from_utf8 or
	/// str::from_utf8.
	...
	Overflow { source: OverflowError, backtrace: BT },
	#[error("Divide by zero: {source}")]
	DivideByZero { source: DivideByZeroError, backtrace: BT },
	#[error("Conversion error: ")]
	ConversionOverflow { source: ConversionOverflowError, backtrace: BT },
}

impl_from_err!(ConversionOverflowError, StdError, StdError::ConversionOverflow);

impl StdError {
	pub fn verification_err(source: VerificationError) -> Self {
		StdError::VerificationErr { source, backtrace: BT::capture() }
	}

	pub fn recover_pubkey_err(source: RecoverPubkeyError) -> Self {
		StdError::RecoverPubkeyErr { source, backtrace: BT::capture() }
	}

	pub fn generic_err(msg: impl Into<String>) -> Self {
		StdError::GenericErr { msg: msg.into(), backtrace: BT::capture() }
	}

	pub fn invalid_base64(msg: impl ToString) -> Self {
		StdError::InvalidBase64 { msg: msg.to_string(), backtrace: BT::capture() }
	}
	...
	pub fn invalid_hex(msg: impl ToString) -> Self {
		StdError::InvalidHex { msg: msg.to_string(), backtrace: BT::capture() }
	}

	pub fn invalid_utf8(msg: impl ToString) -> Self {
		StdError::InvalidUtf8 { msg: msg.to_string(), backtrace: BT::capture() }
	}

	pub fn not_found(kind: impl Into<String>) -> Self {
		StdError::NotFound { kind: kind.into(), backtrace: BT::capture() }
	}
	...
	pub fn overflow(source: OverflowError) -> Self {
		StdError::Overflow { source, backtrace: BT::capture() }
	}

	pub fn divide_by_zero(source: DivideByZeroError) -> Self {
		StdError::DivideByZero { source, backtrace: BT::capture() }
	}
}
...
impl PartialEq<StdError> for StdError {
	fn eq(&self, rhs: &StdError) -> bool {
		match self {
			StdError::VerificationErr { source, backtrace: _ } =>
				if let StdError::VerificationErr { source: rhs_source, backtrace: _ } = rhs {
					source == rhs_source
				} else {
					false
				},
			StdError::RecoverPubkeyErr { source, backtrace: _ } =>
				if let StdError::RecoverPubkeyErr { source: rhs_source, backtrace: _ } = rhs {
					source == rhs_source
				} else {
					false
				},
			StdError::GenericErr { msg, backtrace: _ } => {
				if let StdError::GenericErr { msg: rhs_msg, backtrace: _ } = rhs {
					msg == rhs_msg
				} else {
					false
				}
			...
			StdError::InvalidBase64 { msg, backtrace: _ } => {
				if let StdError::InvalidBase64 { msg: rhs_msg, backtrace: _ } = rhs {
					msg == rhs_msg
				} else {
					false
				}
			},
			StdError::InvalidDataSize { expected, actual, backtrace: _ } =>
			...
			StdError::InvalidHex { msg, backtrace: _ } => {
				if let StdError::InvalidHex { msg: rhs_msg, backtrace: _ } = rhs {
					msg == rhs_msg
				} else {
					false
				}
			...
			StdError::InvalidUtf8 { msg, backtrace: _ } => {
				if let StdError::InvalidUtf8 { msg: rhs_msg, backtrace: _ } = rhs {
					msg == rhs_msg
				} else {
					false
				}
			...
			StdError::NotFound { kind, backtrace: _ } => {
				if let StdError::NotFound { kind: rhs_kind, backtrace: _ } = rhs {
					kind == rhs_kind
				} else {
					false
				}
			},
			StdError::ParseErr { target_type, msg, backtrace: _ } => {
			...
			StdError::SerializeErr { source_type, msg, backtrace: _ } =>
			...
			StdError::Overflow { source, backtrace: _ } => {
				if let StdError::Overflow { source: rhs_source, backtrace: _ } = rhs {
					source == rhs_source
				} else {
					false
				}
			},
			StdError::DivideByZero { source, backtrace: _ } => {
				if let StdError::DivideByZero { source: rhs_source, backtrace: _ } = rhs {
					source == rhs_source
				} else {
					false
				}
			},
			StdError::ConversionOverflow { source, backtrace: _ } =>
				if let StdError::ConversionOverflow { source: rhs_source, backtrace: _ } = rhs {
					source == rhs_source
				} else {
					false
				},
...
///
/// The prefix "Core"/"Std" means "the standard result within the core/standard library". This is
/// not the only result/error type in cosmwasm-core/cosmwasm-std.
...
mod tests {
	...
	fn underflow_works_for_u128() {
		...
		assert!(matches!(
			error,
			StdError::Overflow { source: OverflowError { operation: OverflowOperation::Sub }, .. }
		));
	}
	...
	fn divide_by_zero_works() {
		...
		assert!(matches!(error, StdError::DivideByZero { source: DivideByZeroError, .. }));
	}
	...
	fn implements_display() {
		...
		assert_eq!(embedded, "Display: Overflow: Cannot Sub with given operands");
	}
	...
	fn from_std_string_from_utf8error_works() {
		let error: StdError =
			String::from_utf8(b"Hello \xF0\x90\x80World".to_vec()).unwrap_err().into();
		match error {
			StdError::InvalidUtf8 { msg, .. } => {
				assert_eq!(msg, "invalid utf-8 sequence of 3 bytes from index 6")
			},
