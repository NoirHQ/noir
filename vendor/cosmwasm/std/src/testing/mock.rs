use crate::{prelude::*, Addr, CanonicalAddr, HashFunction, Timestamp};
...
use crate::{
	...
	deps::OwnedDeps,
	from_json,
	query::{AllBalanceResponse, BalanceResponse, BankQuery, CustomQuery, QueryRequest, WasmQuery},
	results::{ContractResult, Empty, SystemResult},
	storage::MemoryStorage,
	to_json_binary,
	traits::{Api, Querier, QuerierResult},
	types::{BlockInfo, ContractInfo, Env, MessageInfo, TransactionInfo},
	Attribute, Binary, DenomMetadata, RecoverPubkeyError, StdError, StdResult, SystemError,
...
// not really smart, but allows us to see a difference (and consistent length for canonical
// addresses)
#[derive(Copy, Clone)]
pub struct MockApi {
...
impl Default for MockApi {
	fn default() -> Self {
		MockApi { bech32_prefix: BECH32_PREFIX }
	}
}
...
impl Api for MockApi {
	...
	fn addr_canonicalize(&self, input: &str) -> StdResult<CanonicalAddr> {
		...
		if !hrp_str.hrp().as_bytes().eq_ignore_ascii_case(self.bech32_prefix.as_bytes()) {
	...
	fn bls12_381_hash_to_g1(
		...
		Ok(cosmwasm_crypto::bls12_381_hash_to_g1(hash_function.into(), msg, dst))
	}
	...
	fn bls12_381_hash_to_g2(
		...
		Ok(cosmwasm_crypto::bls12_381_hash_to_g2(hash_function.into(), msg, dst))
	}
	...
	fn secp256k1_verify(
		...
		Ok(cosmwasm_crypto::secp256k1_verify(message_hash, signature, public_key)?)
	}
	...
	fn secp256r1_verify(
		...
		Ok(cosmwasm_crypto::secp256r1_verify(message_hash, signature, public_key)?)
	}
	...
	fn ed25519_verify(
		...
		Ok(cosmwasm_crypto::ed25519_verify(message, signature, public_key)?)
	}
	...
	fn ed25519_batch_verify(
		...
		Ok(cosmwasm_crypto::ed25519_batch_verify(&mut OsRng, messages, signatures, public_keys)?)
	}
...
pub fn mock_env() -> Env {
	Env {
		...
		contract: ContractInfo { address: Addr::unchecked(MOCK_CONTRACT_ADDR) },
	}
}
...
/// Creates a IbcPacketReceiveMsg for testing ibc_packet_receive. You set a few key parameters that
/// are often parsed. If you want to set more, use this as a default and mutate other fields
#[cfg(feature = "stargate")]
pub fn mock_ibc_packet_recv(
	...
	Ok(IbcPacketReceiveMsg::new(
		IbcPacket {
			...
			dest: IbcEndpoint { port_id: "our-port".to_string(), channel_id: my_channel_id.into() },
			sequence: 27,
			timeout: IbcTimeoutBlock { revision: 1, height: 12345678 }.into(),
		},
...
fn mock_ibc_packet(my_channel_id: &str, data: &impl Serialize) -> StdResult<IbcPacket> {
	Ok(IbcPacket {
		data: to_json_binary(data)?,
		src: IbcEndpoint { port_id: "their-port".to_string(), channel_id: my_channel_id.into() },
		dest: IbcEndpoint {
			port_id: "our-port".to_string(),
			channel_id: "channel-1234".to_string(),
		},
		sequence: 29,
		timeout: IbcTimeoutBlock { revision: 1, height: 432332552 }.into(),
	})
}
...
pub fn mock_ibc_packet_ack(
	...
	Ok(IbcPacketAckMsg::new(ack, packet, Addr::unchecked("relayer")))
}

/// Creates a IbcPacketTimeoutMsg for testing ibc_packet_timeout. You set a few key parameters that
/// are often parsed. If you want to set more, use this as a default and mutate other fields.
/// The difference from mock_ibc_packet_recv is if `my_channel_id` is src or dest.
#[cfg(feature = "stargate")]
	...
	pub fn new(balances: &[(&str, &[Coin])]) -> Self {
		MockQuerier {
			...
			custom_handler: Box::from(|_: &_| -> MockQuerierCustomHandlerResult {
				SystemResult::Err(SystemError::UnsupportedRequest { kind: "custom".to_string() })
			}),
		}
	}
...
impl<C: CustomQuery + DeserializeOwned> MockQuerier<C> {
	pub fn handle_query(&self, request: &QueryRequest<C>) -> QuerierResult {
		match &request {
			...
			#[cfg(feature = "cosmwasm_1_3")]
			QueryRequest::Distribution(distribution_query) => self.distribution.query(distribution_query),
			QueryRequest::Wasm(msg) => self.wasm.query(msg),
			#[cfg(feature = "stargate")]
			#[allow(deprecated)]
			QueryRequest::Stargate { .. } =>
				SystemResult::Err(SystemError::UnsupportedRequest { kind: "Stargate".to_string() }),
			#[cfg(feature = "cosmwasm_2_0")]
			QueryRequest::Grpc(_) =>
				SystemResult::Err(SystemError::UnsupportedRequest { kind: "GRPC".to_string() }),
			#[cfg(feature = "stargate")]
		...
		let handler = Box::from(|request: &WasmQuery| -> QuerierResult {
			let err = match request {
				WasmQuery::Smart { contract_addr, .. } =>
					SystemError::NoSuchContract { addr: contract_addr.clone() },
				WasmQuery::Raw { contract_addr, .. } =>
					SystemError::NoSuchContract { addr: contract_addr.clone() },
				WasmQuery::ContractInfo { contract_addr, .. } =>
					SystemError::NoSuchContract { addr: contract_addr.clone() },
				#[cfg(feature = "cosmwasm_1_2")]
				WasmQuery::CodeInfo { code_id, .. } => SystemError::NoSuchCode { code_id: *code_id },
			};
...
impl BankQuerier {
	pub fn new(balances: &[(&str, &[Coin])]) -> Self {
		let balances: BTreeMap<_, _> =
			balances.iter().map(|(s, c)| (s.to_string(), c.to_vec())).collect();
	...
	pub fn set_denom_metadata(&mut self, denom_metadata: &[DenomMetadata]) {
		self.denom_metadata =
			denom_metadata.iter().map(|d| (d.base.as_bytes().to_vec(), d.clone())).collect();
	}
	...
	fn calculate_supplies(balances: &BTreeMap<String, Vec<Coin>>) -> BTreeMap<String, Uint128> {
		...
		for coin in all_coins {
			*supplies.entry(coin.denom.clone()).or_insert_with(Uint128::zero) += coin.amount;
		}
	...
	pub fn query(&self, request: &BankQuery) -> QuerierResult {
		let contract_result: ContractResult<Binary> = match request {
			...
			BankQuery::Supply { denom } => {
				let amount = self.supplies.get(denom).cloned().unwrap_or_else(Uint128::zero);
				let bank_res = SupplyResponse { amount: Coin { amount, denom: denom.to_string() } };
			...
			BankQuery::Balance { address, denom } => {
				...
				let amount = self
					...
					.unwrap_or_default();
				let bank_res =
					BalanceResponse { amount: Coin { amount, denom: denom.to_string() } };
				to_json_binary(&bank_res).into()
			},
			...
			BankQuery::DenomMetadata { denom } => {
				...
				match denom_metadata {
					Some(m) => {
						let metadata_res = DenomMetadataResponse { metadata: m.clone() };
			...
			BankQuery::AllDenomMetadata { pagination } => {
				let default_pagination = PageRequest { key: None, limit: 100, reverse: false };
				...
				// using dynamic dispatch here to reduce code duplication and since this is only
				// testing code
				let iter: Box<dyn Iterator<Item = _>> =
					if pagination.reverse { Box::new(iter.rev()) } else { Box::new(iter) };
...
impl IbcQuerier {
	...
	pub fn new(port_id: &str, channels: &[IbcChannel]) -> Self {
		IbcQuerier { port_id: port_id.to_string(), channels: channels.to_vec() }
	}
	...
	pub fn query(&self, request: &IbcQuery) -> QuerierResult {
		let contract_result: ContractResult<Binary> = match request {
			IbcQuery::Channel { channel_id, port_id } => {
				let channel = self
					.channels
					.iter()
			...
			IbcQuery::PortId {} => {
				let res = PortIdResponse { port_id: self.port_id.clone() };
				to_json_binary(&res).into()
			},
		};
		// system result is always ok in the mock implementation
	...
	pub fn query(&self, request: &StakingQuery) -> QuerierResult {
		let contract_result: ContractResult<Binary> = match request {
			StakingQuery::BondedDenom {} => {
				let res = BondedDenomResponse { denom: self.denom.clone() };
				to_json_binary(&res).into()
			},
			StakingQuery::AllValidators {} => {
				let res = AllValidatorsResponse { validators: self.validators.clone() };
				to_json_binary(&res).into()
			},
			StakingQuery::Validator { address } => {
				let validator: Option<Validator> =
					self.validators.iter().find(|validator| validator.address == *address).cloned();
			...
			StakingQuery::Delegation { delegator, validator } => {
				...
				let res = DelegationResponse { delegation: delegation.cloned() };
				to_json_binary(&res).into()
			},
		};
		// system result is always ok in the mock implementation
...
impl DistributionQuerier {
	...
	pub fn set_validators(
		...
		self.validators
			.insert(delegator.into(), validators.into_iter().map(Into::into).collect());
	}
		...
		let contract_result: ContractResult<Binary> = match request {
			DistributionQuery::DelegatorWithdrawAddress { delegator_address } => {
				let res = DelegatorWithdrawAddressResponse {
					withdraw_address: Addr::unchecked(
						self.withdraw_addresses.get(delegator_address).unwrap_or(delegator_address),
					),
				};
				to_json_binary(&res).into()
			},
			#[cfg(feature = "cosmwasm_1_4")]
			DistributionQuery::DelegationRewards { delegator_address, validator_address } => {
			...
			DistributionQuery::DelegationTotalRewards { delegator_address } => {
				let validator_rewards =
					self.validator_rewards(delegator_address).unwrap_or_default();
...
mod tests {
	...
	fn mock_info_works() {
		...
		assert_eq!(
			...
			MessageInfo {
				...
				funds: vec![Coin { amount: 100u128.into(), denom: "atom".into() }]
			}
		);
	}
	...
	fn bls12_381_pairing_equality_works() {
		...
		.into_iter()
		.flat_map(|msg| api.bls12_381_hash_to_g2(HashFunction::Sha256, &msg, dst).unwrap())
		.collect();
		...
		let is_valid =
			api.bls12_381_pairing_equality(&ps, &qs, &BLS12_381_G1_GENERATOR, &s).unwrap();
		assert!(is_valid);
	}
	...
	fn bls12_381_hash_to_g1_works() {
		...
		let hashed_point = api.bls12_381_hash_to_g1(HashFunction::Sha256, msg, dst).unwrap();
	...
	fn bls12_381_hash_to_g2_works() {
		...
		let hashed_point = api.bls12_381_hash_to_g2(HashFunction::Sha256, msg, dst).unwrap();
	...
	fn bls12_318_pairing_equality_works() {
		...
		let msg_point =
			api.bls12_381_hash_to_g2(HashFunction::Sha256, &msg, DOMAIN_HASH_TO_G2).unwrap();
	...
	fn secp256k1_verify_works() {
		...
		assert!(api.secp256k1_verify(&hash, &signature, &public_key).unwrap());
	}
	...
	fn secp256k1_recover_pubkey_works() {
		...
		let pubkey = api.secp256k1_recover_pubkey(&hash, &signature, recovery_param).unwrap();
	...
	fn secp256r1_verify_works() {
		...
		assert!(api.secp256r1_verify(&hash, &signature, &public_key).unwrap());
	}
	...
	fn secp256r1_recover_pubkey_works() {
		...
		let pubkey = api.secp256r1_recover_pubkey(&hash, &signature, recovery_param).unwrap();
		assert_eq!(pubkey, expected);
	}
	...
	fn ed25519_batch_verify_works() {
		...
		assert!(api.ed25519_batch_verify(&msgs, &signatures, &public_keys).unwrap());
	}
	...
	fn bank_querier_supply() {
		...
		let elf = bank.query(&BankQuery::Supply { denom: "ELF".to_string() }).unwrap().unwrap();
		let res: SupplyResponse = from_json(elf).unwrap();
		assert_eq!(res.amount, coin(444, "ELF"));

		let fly = bank.query(&BankQuery::Supply { denom: "FLY".to_string() }).unwrap().unwrap();
		...
		let atom = bank.query(&BankQuery::Supply { denom: "ATOM".to_string() }).unwrap().unwrap();
	...
	fn bank_querier_all_balances() {
		...
		let all = bank.query(&BankQuery::AllBalances { address: addr }).unwrap().unwrap();
	...
	fn bank_querier_one_balance() {
		...
		// one match
		let fly = bank
			.query(&BankQuery::Balance { address: addr.clone(), denom: "FLY".to_string() })
			.unwrap()
			.unwrap();
		...
		// missing denom
		let miss = bank
			.query(&BankQuery::Balance { address: addr, denom: "MISS".to_string() })
			.unwrap()
			.unwrap();
	...
	fn bank_querier_missing_account() {
		...
		let all = bank
			.query(&BankQuery::AllBalances { address: String::from("elsewhere") })
			.unwrap()
			.unwrap();
		let res: AllBalanceResponse = from_json(all).unwrap();
	...
	fn bank_querier_metadata_works() {
		...
		let res = bank
			.query(&BankQuery::AllDenomMetadata {
				pagination: Some(PageRequest { key: None, limit: 10, reverse: false }),
			})
			.unwrap()
			.unwrap();
		...
		let res2 = bank
			.query(&BankQuery::AllDenomMetadata {
				pagination: Some(PageRequest { key: res.next_key, limit: 10, reverse: false }),
			})
			.unwrap()
			.unwrap();
		...
		let res = bank
			.query(&BankQuery::AllDenomMetadata {
				pagination: Some(PageRequest { key: None, limit: 100, reverse: true }),
			})
			.unwrap()
			.unwrap();
		let res: AllDenomMetadataResponse = from_json(res).unwrap();
		...
		let more_res = bank
			.query(&BankQuery::AllDenomMetadata {
				pagination: Some(PageRequest { key: res.next_key, limit: u32::MAX, reverse: true }),
			})
			.unwrap()
			.unwrap();
		let more_res: AllDenomMetadataResponse = from_json(more_res).unwrap();
		assert_eq!(more_res.metadata, res.metadata, "should be same as previous query");
	}
	...
	fn distribution_querier_delegator_withdraw_address() {
		...
		let query =
			DistributionQuery::DelegatorWithdrawAddress { delegator_address: "addr0".to_string() };
		...
		let query =
			DistributionQuery::DelegatorWithdrawAddress { delegator_address: "addr1".to_string() };
	...
	fn distribution_querier_delegator_validators() {
		...
		let query =
			DistributionQuery::DelegatorValidators { delegator_address: "addr0".to_string() };
		...
		let query =
			DistributionQuery::DelegatorValidators { delegator_address: "addr1".to_string() };
	...
	fn distribution_querier_delegation_rewards() {
		...
		// total rewards
		let query =
			DistributionQuery::DelegationTotalRewards { delegator_address: "addr0".to_string() };
		let res = distribution.query(&query).unwrap().unwrap();
		let res: DelegationTotalRewardsResponse = from_json(res).unwrap();
		assert_eq!(
			res.rewards,
			vec![
				DelegatorReward { validator_address: "valoper0".into(), reward: valoper0_rewards },
				DelegatorReward { validator_address: "valoper1".into(), reward: valoper1_rewards },
			]
		);
		assert_eq!(
			res.total,
	...
	fn ibc_querier_channel_none() {
		...
		// query non-existing
		let query = &IbcQuery::Channel { channel_id: "channel-0".to_string(), port_id: None };
		let raw = ibc.query(query).unwrap().unwrap();
		let chan: ChannelResponse = from_json(raw).unwrap();
	...
	fn ibc_querier_channels_matching() {
		...
		let query = &IbcQuery::ListChannels { port_id: Some("my_port".to_string()) };
		let raw = ibc.query(query).unwrap().unwrap();
	...
	fn staking_querier_all_validators() {
		...
		// one match
		let raw = staking.query(&StakingQuery::AllValidators {}).unwrap().unwrap();
	...
	fn staking_querier_validator() {
		...
		// query 1
		let raw = staking.query(&StakingQuery::Validator { address: address1 }).unwrap().unwrap();
		let res: ValidatorResponse = from_json(raw).unwrap();
		assert_eq!(res.validator, Some(val1));

		// query 2
		let raw = staking.query(&StakingQuery::Validator { address: address2 }).unwrap().unwrap();
		let res: ValidatorResponse = from_json(raw).unwrap();
		assert_eq!(res.validator, Some(val2));

		// query non-existent
		let raw = staking
			.query(&StakingQuery::Validator { address: address_non_existent })
			.unwrap()
			.unwrap();
		let res: ValidatorResponse = from_json(raw).unwrap();
	...
	fn get_all_delegators(
		...
		let raw = staking
			.query(&StakingQuery::AllDelegations { delegator: delegator.into() })
			.unwrap()
			.unwrap();
	...
	fn wasm_querier_works() {
		...
		let system_err = querier
			.query(&WasmQuery::Raw { contract_addr: any_addr.clone(), key: b"the key".into() })
			.unwrap_err();
		match system_err {
		...
		let system_err = querier
			.query(&WasmQuery::Smart { contract_addr: any_addr.clone(), msg: b"{}".into() })
			.unwrap_err();
		match system_err {
			SystemError::NoSuchContract { addr } => assert_eq!(addr, any_addr),
			err => panic!("Unexpected error: {err:?}"),
		...
		let system_err = querier
			.query(&WasmQuery::ContractInfo { contract_addr: any_addr.clone() })
			.unwrap_err();
		match system_err {
			SystemError::NoSuchContract { addr } => assert_eq!(addr, any_addr),
			err => panic!("Unexpected error: {err:?}"),
		...
		#[cfg(feature = "cosmwasm_1_2")]
		{
			...
			let system_err = querier.query(&WasmQuery::CodeInfo { code_id: 4 }).unwrap_err();
			match system_err {
		...
		let result =
			querier.query(&WasmQuery::ContractInfo { contract_addr: contract_addr.into() });
		match result {
			SystemResult::Ok(ContractResult::Ok(value)) => assert_eq!(
				value,
	...
	fn colon_in_prefix_is_valid() {
		...
		assert_eq!(addr.as_str(), "did:com:1jkf0kmeyefvyzpwf56m7sne2000ay53r6upttu");
	}
	...
	fn instantiate2_address_can_be_humanized() {
		...
		let contract_addr =
			mock_api.addr_canonicalize(mock_api.addr_make("contract").as_str()).unwrap();
		let checksum =
