...
use crate::{
	deps::OwnedDeps,
	ibc::{
		IbcBasicResponse, IbcChannelOpenMsg, IbcChannelOpenResponse, IbcDestinationCallbackMsg,
		IbcSourceCallbackMsg,
	},
	imports::{ExternalApi, ExternalQuerier, ExternalStorage},
	memory::{Owned, Region},
	query::CustomQuery,
	results::{ContractResult, QueryResponse, Reply, Response},
	serde::{from_json, to_json_vec},
	types::Env,
	CustomMsg, Deps, DepsMut, MessageInfo,
};
...
/// This should be wrapped in an external "C" export, containing a contract-specific function as an
/// argument.
///
...
/// do_execute should be wrapped in an external "C" export, containing a contract-specific function
/// as arg
///
/// - `Q`: custom query type (see QueryRequest)
/// - `M`: message type for request
...
/// do_migrate should be wrapped in an external "C" export, containing a contract-specific function
/// as arg
///
/// - `Q`: custom query type (see QueryRequest)
/// - `M`: message type for request
	...
	E: ToString,
{
	#[cfg(feature = "abort")]
	install_panic_handler();
	let res = _do_migrate(migrate_fn, env_ptr as *mut Region<Owned>, msg_ptr as *mut Region<Owned>);
	let v = to_json_vec(&res).unwrap();
	Region::from_vec(v).to_heap_ptr() as u32
}

/// do_sudo should be wrapped in an external "C" export, containing a contract-specific function as
/// arg
///
/// - `Q`: custom query type (see QueryRequest)
/// - `M`: message type for request
	...
	E: ToString,
{
	#[cfg(feature = "abort")]
	install_panic_handler();
	let res = _do_sudo(sudo_fn, env_ptr as *mut Region<Owned>, msg_ptr as *mut Region<Owned>);
	let v = to_json_vec(&res).unwrap();
	Region::from_vec(v).to_heap_ptr() as u32
}

/// do_reply should be wrapped in an external "C" export, containing a contract-specific function as
/// arg message body is always `SubcallResult`
///
/// - `Q`: custom query type (see QueryRequest)
/// - `C`: custom response message type (see CosmosMsg)
/// - `E`: error type for responses
	...
	E: ToString,
{
	#[cfg(feature = "abort")]
	install_panic_handler();
	let res = _do_reply(reply_fn, env_ptr as *mut Region<Owned>, msg_ptr as *mut Region<Owned>);
	let v = to_json_vec(&res).unwrap();
	Region::from_vec(v).to_heap_ptr() as u32
}

/// do_query should be wrapped in an external "C" export, containing a contract-specific function as
/// arg
///
/// - `Q`: custom query type (see QueryRequest)
/// - `M`: message type for request
	...
	E: ToString,
{
	#[cfg(feature = "abort")]
	install_panic_handler();
	let res = _do_query(query_fn, env_ptr as *mut Region<Owned>, msg_ptr as *mut Region<Owned>);
	let v = to_json_vec(&res).unwrap();
	Region::from_vec(v).to_heap_ptr() as u32
}
