...
use crate::{prelude::*, traits::Storage};
...
#[cfg(feature = "iterator")]
/// The BTreeMap specific key-value pair reference type, as returned by BTreeMap<Vec<u8>,
/// Vec<u8>>::range. This is internal as it can change any time if the map implementation is swapped
/// out.
...
mod tests {
	...
	fn iterator() {
		...
		// bounded (descending)
		{
			...
			assert_eq!(
				elements,
				vec![(b"foo".to_vec(), b"bar".to_vec()), (b"ant".to_vec(), b"hill".to_vec()),]
			);
		}
		...
		// right unbounded
		{
			...
			assert_eq!(
				elements,
				vec![(b"foo".to_vec(), b"bar".to_vec()), (b"ze".to_vec(), b"bra".to_vec()),]
			);
		}
		...
		// right unbounded (descending)
		{
			...
			assert_eq!(
				elements,
				vec![(b"ze".to_vec(), b"bra".to_vec()), (b"foo".to_vec(), b"bar".to_vec()),]
			);
		}
