...
use crate::{
	coin::Coin,
	prelude::*,
	results::{Attribute, CosmosMsg, Empty, Event, SubMsg},
	to_json_binary, Addr, Binary, StdResult, Timestamp,
};
...
pub enum IbcMsg {
	...
	/// This allows acknowledging a packet that was not acknowledged yet in the
	/// `ibc_packet_receive` call.
...
impl IbcTimeout {
	pub fn with_block(block: IbcTimeoutBlock) -> Self {
		IbcTimeout { block: Some(block), timestamp: None }
	}

	pub fn with_timestamp(timestamp: Timestamp) -> Self {
		IbcTimeout { block: None, timestamp: Some(timestamp) }
	}

	pub fn with_both(block: IbcTimeoutBlock, timestamp: Timestamp) -> Self {
		IbcTimeout { block: Some(block), timestamp: Some(timestamp) }
	}
...
impl IbcPacket {
	...
	pub fn new(
		...
		Self { data: data.into(), src, dest, sequence, timeout }
	}
}
...
impl IbcAcknowledgement {
	...
	pub fn encode_json(data: &impl Serialize) -> StdResult<Self> {
		Ok(IbcAcknowledgement { data: to_json_binary(data)? })
	}
}
...
pub enum IbcChannelOpenMsg {
	...
	OpenTry { channel: IbcChannel, counterparty_version: String },
}
...
impl IbcChannelOpenMsg {
	...
	pub fn new_try(channel: IbcChannel, counterparty_version: impl Into<String>) -> Self {
		Self::OpenTry { channel, counterparty_version: counterparty_version.into() }
	}
	...
	pub fn counterparty_version(&self) -> Option<&str> {
		match self {
			Self::OpenTry { counterparty_version, .. } => Some(counterparty_version),
			_ => None,
		}
	}
}
...
pub enum IbcChannelConnectMsg {
	...
	OpenAck { channel: IbcChannel, counterparty_version: String },
...
impl IbcChannelConnectMsg {
	pub fn new_ack(channel: IbcChannel, counterparty_version: impl Into<String>) -> Self {
		Self::OpenAck { channel, counterparty_version: counterparty_version.into() }
	}
	...
	pub fn counterparty_version(&self) -> Option<&str> {
		match self {
			Self::OpenAck { counterparty_version, .. } => Some(counterparty_version),
			_ => None,
		}
	}
}
...
impl IbcPacketAckMsg {
	pub fn new(
		...
		Self { acknowledgement, original_packet, relayer }
	}
}
...
impl<T> Default for IbcBasicResponse<T> {
	fn default() -> Self {
		IbcBasicResponse { messages: vec![], attributes: vec![], events: vec![] }
	}
}
...
impl<T> IbcReceiveResponse<T> {
	...
	pub fn without_ack() -> Self {
		Self { acknowledgement: None, messages: vec![], attributes: vec![], events: vec![] }
	}
...
mod tests {
	...
	#[test]
	// added this to check json format for go compat, as I was unsure how some messages are snake
	// encoded
	fn serialize_msg() {
	...
	fn ibc_timeout_serialize() {
		...
		let block = IbcTimeout::with_block(IbcTimeoutBlock { revision: 12, height: 129 });
		...
		let both = IbcTimeout::with_both(
			IbcTimeoutBlock { revision: 12, height: 129 },
	...
	fn ibc_timeout_block_ord() {
		let epoch1a = IbcTimeoutBlock { revision: 1, height: 1000 };
		let epoch1b = IbcTimeoutBlock { revision: 1, height: 3000 };
		let epoch2a = IbcTimeoutBlock { revision: 2, height: 500 };
		let epoch2b = IbcTimeoutBlock { revision: 2, height: 2500 };
	...
	fn ibc_packet_serialize() {
		let packet = IbcPacket {
			...
			dest: IbcEndpoint { port_id: "our-port".to_string(), channel_id: "chan33".into() },
			sequence: 27,
			timeout: IbcTimeout::with_both(
				IbcTimeoutBlock { revision: 1, height: 12345678 },
		...
		let no_timestamp = IbcPacket {
			...
			dest: IbcEndpoint { port_id: "our-port".to_string(), channel_id: "chan33".into() },
			sequence: 27,
			timeout: IbcTimeout::with_block(IbcTimeoutBlock { revision: 1, height: 12345678 }),
		};
