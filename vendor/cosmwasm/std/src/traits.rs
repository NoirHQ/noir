...
use crate::{
	...
	to_json_binary, to_json_vec, Addr, Binary, CanonicalAddr, ContractInfoResponse,
...
pub trait Storage {
	...
	/// If `start` is lexicographically greater than or equal to `end`, an empty range is described,
	/// mo matter of the order.
	#[cfg(feature = "iterator")]
	...
	fn range<'a>(
		...
		// This default implementation is just to avoid breaking code when enabling the `iterator`
		// feature. Any actual `Storage` impl should override this method.
	...
	/// mo matter of the order.
	///
	/// The default implementation uses [`Storage::range`] and discards the keys. More efficient
	/// implementations might be possible depending on the storage.
	#[cfg(feature = "iterator")]
	fn range_values<'a>(
		&'a self,
...
pub trait Api {
	...
	/// 1. The address is valid in the sense that it can be converted to a canonical representation
	///    by the backend.
	...
	fn bls12_381_aggregate_g1(&self, g1s: &[u8]) -> Result<[u8; 48], VerificationError> {
		// Support for BLS12-381 is added in 2.1, i.e. we can't add a compile time requirement for
		// new function. Any implementation of the Api trait which does not implement this
		// function but tries to call it will panic at runtime. We don't assume such cases exist.
	...
	///
	/// The argument `ps` contain the points p_1, ..., p_n ∈ G1 as a concatenation of 48 byte
	/// elements. The argument `qs` contain the points q_1, ..., q_n ∈ G2 as a concatenation of 96
	/// byte elements.
	///
	/// ## Examples
	///
	...
	fn secp256r1_verify(
		...
		// Support for secp256r1 is added in 2.1, i.e. we can't add a compile time requirement for
		// new function. Any implementation of the Api trait which does not implement this
		// function but tries to call it will panic at runtime. We don't assume such cases exist.
		// See also https://doc.rust-lang.org/cargo/reference/semver.html#trait-new-default-item
		unimplemented!()
	...
	fn secp256r1_recover_pubkey(
		...
		// Support for secp256r1 was added in 2.1, i.e. we can't add a compile time requirement for
		// new function. Any implementation of the Api trait which does not implement this
		// function but tries to call it will panic at runtime. We don't assume such cases exist.
		// See also https://doc.rust-lang.org/cargo/reference/semver.html#trait-new-default-item
		unimplemented!()
	...
	/// Emits a debugging message that is handled depending on the environment (typically printed to
	/// console or ignored). Those messages are not persisted to chain.
	fn debug(&self, message: &str);
}

/// A short-hand alias for the two-level query result (1. accessing the contract, 2. executing query
/// in the contract)
...
impl<'a, C: CustomQuery> QuerierWrapper<'a, C> {
	pub fn new(querier: &'a dyn Querier) -> Self {
		QuerierWrapper { querier, custom_query_type: PhantomData }
	}
	...
	pub fn into_empty(self) -> QuerierWrapper<'a, Empty> {
		QuerierWrapper { querier: self.querier, custom_query_type: PhantomData }
	}
	...
	pub fn query_supply(&self, denom: impl Into<String>) -> StdResult<Coin> {
		let request = BankQuery::Supply { denom: denom.into() }.into();
	...
	pub fn query_balance(
		...
		let request = BankQuery::Balance { address: address.into(), denom: denom.into() }.into();
		let res: BalanceResponse = self.query(&request)?;
		Ok(res.amount)
	}

	pub fn query_all_balances(&self, address: impl Into<String>) -> StdResult<Vec<Coin>> {
		let request = BankQuery::AllBalances { address: address.into() }.into();
	...
	pub fn query_delegator_withdraw_address(
		...
		let request =
			DistributionQuery::DelegatorWithdrawAddress { delegator_address: delegator.into() }
				.into();
	...
	pub fn query_denom_metadata(&self, denom: impl Into<String>) -> StdResult<DenomMetadata> {
		let request = BankQuery::DenomMetadata { denom: denom.into() }.into();
	...
	pub fn query_all_denom_metadata(
		...
		let request = BankQuery::AllDenomMetadata { pagination: Some(pagination) }.into();
		self.query(&request)
	}
	...
	pub fn query_delegation_total_rewards(
		...
		let request =
			DistributionQuery::DelegationTotalRewards { delegator_address: delegator.into() }
				.into();
		self.query(&request)
	}
	...
	pub fn query_delegator_validators(
		...
		let request =
			DistributionQuery::DelegatorValidators { delegator_address: delegator.into() }.into();
	...
	pub fn query_wasm_smart<T: DeserializeOwned>(
		...
		let request =
			WasmQuery::Smart { contract_addr: contract_addr.into(), msg: to_json_binary(msg)? }
				.into();
		self.query(&request)
	}
	...
	///
	/// Similar return value to [`Storage::get`]. Returns `Some(val)` or `None` if the data is
	/// there. It only returns error on some runtime issue, not on any data cases.
	pub fn query_wasm_raw(
		&self,
		contract_addr: impl Into<String>,
		key: impl Into<Binary>,
	) -> StdResult<Option<Vec<u8>>> {
		let request: QueryRequest<Empty> =
			WasmQuery::Raw { contract_addr: contract_addr.into(), key: key.into() }.into();
		// we cannot use query, as it will try to parse the binary data, when we just want to return
		// it, so a bit of code copy here...
		let raw = to_json_vec(&request).map_err(|serialize_err| {
			StdError::generic_err(format!("Serializing QueryRequest: {serialize_err}"))
		})?;
		match self.raw_query(&raw) {
	...
	pub fn query_wasm_contract_info(
		...
		let request = WasmQuery::ContractInfo { contract_addr: contract_addr.into() }.into();
		self.query(&request)
	}
	...
	pub fn query_validator(&self, address: impl Into<String>) -> StdResult<Option<Validator>> {
		let request = StakingQuery::Validator { address: address.into() }.into();
	...
	pub fn query_all_delegations(
		...
		let request = StakingQuery::AllDelegations { delegator: delegator.into() }.into();
	...
	pub fn query_delegation(
		...
		let request =
			StakingQuery::Delegation { delegator: delegator.into(), validator: validator.into() }
				.into();
...
mod tests {
	...
	use super::*;
	use crate::{coins, testing::MockQuerier, Uint128};
	...
	fn auto_deref_raw_query() {
		...
		let raw = wrapper.raw_query(&to_json_vec(&query).unwrap()).unwrap().unwrap();
	...
	fn contract_info() {
		...
		querier.update_wasm(|q| -> QuerierResult {
			if q == &(WasmQuery::ContractInfo { contract_addr: ACCT.to_string() }) {
				SystemResult::Ok(ContractResult::Ok(to_json_binary(&mock_resp()).unwrap()))
			} else {
				SystemResult::Err(crate::SystemError::NoSuchContract { addr: ACCT.to_string() })
			}
		});
		let wrapper = QuerierWrapper::<Empty>::new(&querier);
