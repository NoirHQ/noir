...
mod tests {
	use super::*;
	use crate::{assert_hash_works, errors::StdError};
	...
	fn test_base64_encoding_error() {
		for (invalid_base64, want) in
			[("cm%uZG9taVo", "Invalid symbol 37, offset 2."), ("cmFuZ", "Invalid input length: 5")]
		{
	...
	fn into_vec_works() {
		...
		let original = Binary::new(vec![0u8, 187, 61, 11, 250, 0]);
		...
		let original = Binary::new(vec![7u8, 35, 49, 101, 0, 255]);
		let original_ptr = original.0.as_ptr();
	...
	fn serialization_works() {
		let binary = Binary::new(vec![0u8, 187, 61, 11, 250, 0]);
	...
	fn binary_implements_debug() {
		// Some data
		let binary = Binary::new(vec![0x07, 0x35, 0xAA, 0xcb, 0x00, 0xff]);
	...
	fn binary_implements_deref() {
		// Dereference to [u8]
		let binary = Binary::new(vec![7u8, 35, 49, 101, 0, 255]);
	...
	fn binary_implements_as_ref() {
		...
		let data = Binary::new(want.to_vec());
	...
	fn binary_implements_partial_eq_with_slice_and_array() {
		let a = Binary::new(vec![0xAA, 0xBB]);
