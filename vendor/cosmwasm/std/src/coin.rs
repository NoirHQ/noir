...
use crate::{prelude::*, CoinFromStrError, Uint128};
...
impl Coin {
	pub fn new(amount: impl Into<Uint128>, denom: impl Into<String>) -> Self {
		Coin { amount: amount.into(), denom: denom.into() }
	}
}
...
impl FromStr for Coin {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let pos = s.find(|c: char| !c.is_ascii_digit()).ok_or(CoinFromStrError::MissingDenom)?;
		...
		Ok(Coin { amount: amount.parse::<u128>()?.into(), denom: denom.to_string() })
	}
}
...
mod tests {
	...
	fn coin_implements_display() {
		let a = Coin { amount: Uint128::new(123), denom: "ucosm".to_string() };
	...
	fn coin_works() {
		...
		assert_eq!(a, Coin { amount: Uint128::new(123), denom: "ucosm".to_string() });

		let zero = coin(0, "ucosm");
		assert_eq!(zero, Coin { amount: Uint128::new(0), denom: "ucosm".to_string() });

		let string_denom = coin(42, String::from("ucosm"));
		assert_eq!(string_denom, Coin { amount: Uint128::new(42), denom: "ucosm".to_string() });
	}

	#[test]
	fn coins_works() {
		let a = coins(123, "ucosm");
		assert_eq!(a, vec![Coin { amount: Uint128::new(123), denom: "ucosm".to_string() }]);

		let zero = coins(0, "ucosm");
		assert_eq!(zero, vec![Coin { amount: Uint128::new(0), denom: "ucosm".to_string() }]);

		let string_denom = coins(42, String::from("ucosm"));
		assert_eq!(
			string_denom,
			vec![Coin { amount: Uint128::new(42), denom: "ucosm".to_string() }]
		);
	}
	...
	fn parse_coin() {
		...
		// error cases
		assert_eq!(Coin::from_str("123").unwrap_err(), CoinFromStrError::MissingDenom);
		assert_eq!(
