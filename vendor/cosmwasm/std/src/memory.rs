...
impl Region<Owned> {
	...
	/// You'll want to use this when you received a region from the VM and want to dereference its
	/// contents.
	///
	/// # Safety
	///
	...
	/// Construct a new empty region with *at least* a capacity of what you passed in and a length
	/// of 0
	...
	O: Ownership,
{
	...
	///
	/// This is nothing but `&self as *const Region<T>` but makes sure the correct generic parameter
	/// is used
	...
	///
	/// This means we move this regions onto the heap (note, only the *structure* of the region, not
	/// the *contents of the pointer* we manage internally). To then deallocate this structure,
	/// you'll have to reconstruct the region via [`Region::from_heap_ptr`] and drop it.
