...
impl<'a> CodeValidation<'a> {
	...
	pub fn validate_exports(self, expected_exports: &[Export]) -> Result<Self, ValidationError> {
		...
		let types = module.type_section().map_or(Default::default(), TypeSection::types);
		let export_entries =
			module.export_section().map_or(Default::default(), ExportSection::entries);
		let func_entries =
			module.function_section().map_or(Default::default(), FunctionSection::entries);
	...
	pub fn validate_imports(
		...
		let types = module.type_section().map_or(Default::default(), TypeSection::types);
		let import_entries =
			module.import_section().map_or(Default::default(), ImportSection::entries);
			...
			for wasm_type in type_section.types() {
				match wasm_type {
					Type::Function(func_type) => {
						let return_type = func_type.results().first();
