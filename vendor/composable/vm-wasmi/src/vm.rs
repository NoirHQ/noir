	...
	S: AsContextMut<UserState = V>,
{
	...
	fn db_scan(
		...
		self.0.as_context_mut().data_mut().db_scan(start, end, order)
	}
	...
	fn set_contract_meta(
		...
		self.0.as_context_mut().data_mut().set_contract_meta(address, new_contract_meta)
	}
	...
	fn continue_query(
		...
		self.0.as_context_mut().data_mut().continue_query(address, message)
	}
	...
	fn continue_reply(
		...
		self.0.as_context_mut().data_mut().continue_reply(message, event_handler)
	}
	...
	fn message_custom(
		...
		self.0.as_context_mut().data_mut().message_custom(message, event_handler)
	}
	...
	fn transfer_from(
		...
		self.0.as_context_mut().data_mut().transfer_from(from, to, funds)
	}
	...
	fn query_contract_info(
		...
		self.0.as_context_mut().data_mut().query_contract_info(address)
	}
	...
	fn gas_checkpoint_push(&mut self, checkpoint: VmGasCheckpoint) -> Result<(), Self::Error> {
		self.0.as_context_mut().data_mut().gas_checkpoint_push(checkpoint)
	}
	...
	fn secp256k1_verify(
		...
		self.0.as_context_mut().data_mut().charge(VmGas::Secp256k1Verify)?;
		self.0
			.as_context_mut()
			.data_mut()
	...
	fn ibc_send_packet(
		...
		self.0.as_context_mut().data_mut().ibc_send_packet(channel_id, data, timeout)
	}

	#[cfg(feature = "stargate")]
	fn ibc_close_channel(&mut self, channel_id: String) -> Result<(), Self::Error> {
		self.charge(VmGas::IbcCloseChannel)?;
		self.0.as_context_mut().data_mut().ibc_close_channel(channel_id)
	}
}
