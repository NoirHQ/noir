	...
	S: AsContextMut<UserState = V>,
{
	...
	let (messages, signatures, public_keys) =
		(decode_sections(&messages), decode_sections(&signatures), decode_sections(&public_keys));
...
where
	...
	S: AsContextMut<UserState = V>,
{
	vm.charge(VmGas::Instrumentation { metered: value as u32 })?;
	Ok(())
}
...
fn encode_sections(sections: &[Vec<u8>]) -> Option<Vec<u8>> {
	...
	sections.iter().try_fold(Vec::with_capacity(out_len), |mut acc, section| {
