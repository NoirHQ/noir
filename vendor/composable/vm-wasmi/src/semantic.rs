...
impl Gas {
	fn new(initial_value: u64) -> Self {
		Gas { checkpoints: vec![initial_value] }
	}
	fn current(&self) -> &u64 {
	...
	fn load_subvm(
		...
		let sub_vm = SimpleWasmiVM {
			...
			env: Env {
				...
				contract: ContractInfo { address: address.into() },
			},
			info: MessageInfo { sender: self.env.contract.address.clone(), funds },
...
impl TryFrom<Vec<u8>> for CanonicalAddress {
	...
	fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
		Ok(CanonicalAddress(CanonicalAddr::from(Binary::from(value))))
	}
}
...
impl<'a> VMBase for SimpleWasmiVM<'a> {
	...
	fn continue_instantiate(
		...
		self.extension.contracts.insert(BankAccount(address), contract_meta);
	...
	fn continue_reply(
		...
		let mut sub_vm =
			self.load_subvm(self.env.contract.address.clone().into_string().try_into()?, vec![])?;
	...
	fn transfer(&mut self, to: &Self::Address, funds: &[Coin]) -> Result<(), Self::Error> {
		log::debug!("Transfer: {:?} -> {:?}\n{:?}", self.env.contract.address, to, funds);
		Ok(())
	}
	...
	fn db_scan(
		...
		let storage = self.extension.storage.get_mut(&contract_addr).unwrap_or(&mut empty);
	...
	fn secp256k1_recover_pubkey(
		...
		Ok(cosmwasm_crypto::secp256k1_recover_pubkey(message_hash, signature, recovery_param)
			.map_err(|_| ()))
	}
	...
	fn ed25519_batch_verify(
		...
		cosmwasm_crypto::ed25519_batch_verify(&mut OsRng, messages, signatures, public_keys)
			.map_err(|_| SimpleVMError::Crypto)
	}
	...
	fn addr_validate(&mut self, input: &str) -> Result<Result<(), Self::Error>, Self::Error> {
		...
		Ok(if account == normalized { Ok(()) } else { Err(SimpleVMError::InvalidAddress) })
	}
	...
	fn addr_canonicalize(
		...
		// Dummy input validation. This is more sophisticated for formats like bech32, where format
		// and checksum are validated.
		if normalized.len() < 3 {
	...
	fn addr_humanize(
		...
		// Shuffle two more times which restored the original value (24 elements are back to
		// original after 20 rounds)
		...
		let Ok(human) = String::from_utf8(trimmed) else {
			return Ok(Err(SimpleVMError::InvalidAddress));
		};
		Ok(BankAccount::try_from(Addr::unchecked(human)).map_err(|_| SimpleVMError::InvalidAddress))
	}
	...
	fn abort(&mut self, message: String) -> Result<(), Self::Error> {
		...
		Err(SimpleVMError::from(WasmiVMError::from(SystemError::ContractExecutionFailure(message))))
	}
	...
	fn gas_ensure_available(&mut self) -> Result<(), Self::Error> {
		let checkpoint =
			self.extension.gas.checkpoints.last().expect("invalis gas checkpoint state");
		if *checkpoint > 0 {
			Ok(())
		} else {
			Err(SimpleVMError::OutOfGas)
	...
	fn ibc_send_packet(
		...
		entry.packets_sent.push(SimpleIBCPacket { channel_id, data, timeout });
		Ok(())
	}
...
impl TryFrom<String> for BankAccount {
	...
	fn try_from(value: String) -> Result<Self, Self::Error> {
		Ok(BankAccount(u128::from_str(&value).map_err(|_| SimpleVMError::InvalidAccountFormat)?))
	}
}
...
fn create_vm(
	...
	let code = extension
		...
		.expect("contract should have been uploaded")
		.clone();
	let vm = SimpleWasmiVM { executing_module: None, env, info, extension, call_depth: 0 };
...
fn create_simple_vm(
	...
	create_vm(
		...
		MessageInfo { sender: sender.into(), funds },
	)
}
...
fn test_bare() {
	...
	let mut extension = SimpleWasmiVMExtension {
		...
		contracts: BTreeMap::from([(
			address,
			CosmwasmContractMeta { code_id: 0x1337, admin: None, label: String::new() },
		)]),
	...
	assert_eq!(
		...
		.unwrap(),
		QueryResult(CosmwasmQueryResult::Ok(Binary::new(
...
mod test_code_gen {
	...
	fn basic() {
		let module: code_gen::WasmModule =
			code_gen::ModuleDefinition::new(vec![], 10, None).unwrap().into();
		let sender = BankAccount(100);
		let address = BankAccount(10_000);
		let funds = vec![];
		let mut extension = SimpleWasmiVMExtension {
...
fn riffle_shuffle_n(mut input: Vec<u8>, n: usize) -> Vec<u8> {
	assert!(input.len() % 2 == 0, "Method only defined for even number of elements");
	let mid = input.len() / 2;
...
fn test_orchestration_base() {
	...
	.unwrap();
	let attributes = vec![
		Attribute { key: "action".into(), value: "mint".into() },
		Attribute { key: "to".into(), value: "10001".into() },
		Attribute { key: "amount".into(), value: "5555".into() },
	];
...
fn test_orchestration_advanced() {
	...
	assert_eq!(
		...
		.unwrap(),
		QueryResult(CosmwasmQueryResult::Ok(Binary::new(
			r#"{"hashed":"K4xL+Gub1930CJU6hdpwf0t3KNk27f5efqy9+YA6iio="}"#.as_bytes().to_vec()
		)))
	);
}
...
fn test_reply() {
	...
	let mut extension = SimpleWasmiVMExtension {
		...
		contracts: BTreeMap::from([
			(address, CosmwasmContractMeta { code_id: 0x1337, admin: None, label: String::new() }),
			(
				hackatom_address,
				CosmwasmContractMeta { code_id: 0x1338, admin: None, label: String::new() },
			),
		]),
	...
	};
	{
		...
		assert!(events.iter().any(|e| e
			.attributes
			.contains(&Attribute { key: "Let the".into(), value: "hacking begin".into() })));
	}
	log::debug!("{:?}", extension.storage);
	{
		...
		let attributes = vec![
			Attribute { key: "action".into(), value: "release".into() },
			Attribute { key: "destination".into(), value: "10000".into() },
			Attribute { key: "action".into(), value: "reflect_subcall".into() },
		];
...
mod cw20_ics20 {
	...
	fn funded(funds: Vec<Coin>, info: MessageInfo) -> MessageInfo {
		MessageInfo { sender: info.sender, funds }
	}

	fn create_channel(channel_id: &str) -> IbcChannel {
		IbcChannel::new(
			IbcEndpoint { port_id: CONTRACT_PORT.into(), channel_id: channel_id.into() },
			IbcEndpoint { port_id: REMOTE_PORT.into(), channel_id: channel_id.into() },
			IbcOrder::Unordered,
			ICS20_VERSION,
			CONNECTION_ID,
		)
	...
	fn reverse_packet(
		channel: IbcChannel,
		Ics20Packet { amount, denom, receiver, sender, memo }: Ics20Packet,
	) -> Ics20Packet {
	...
	fn test_ics20_ibc_orchestration() {
		...
		let info = MessageInfo { sender: sender.into(), funds };
		let mut extension = SimpleWasmiVMExtension {
			storage: BTreeMap::default(),
			codes: BTreeMap::from([(0x1337, code)]),
			contracts: BTreeMap::from([(
...

		{
			...
			assert_matches!(
				cosmwasm_call_serialize::<IbcChannelOpenCall, OwnedWasmiVM<SimpleWasmiVM>, _>(
					&mut vm,
					&IbcChannelOpenMsg::OpenInit { channel: channel.clone() }
				)
				.unwrap(),
...

		{
			...
			let mut vm = create_vm(
				...
				env.clone(),
				funded(
					vec![Coin { denom: "PICA".into(), amount: Uint128::new(1000) }],
					info.clone(),
				),
			)
			.unwrap();
		...
		let (_next_seq, packets_to_dispatch) = extension.ibc.iter().fold(
			(0u64, Vec::with_capacity(extension.ibc.values().map(|x| x.packets_sent.len()).sum())),
