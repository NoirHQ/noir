		...
		fn from(value: ContractResult<Response<T>>) -> Self {
			IbcPacketReceiveResult(match value {
				ContractResult::Ok(response) => ContractResult::Ok(match response.data {
					Some(data) => IbcReceiveResponse::new(data)
						.add_submessages(response.messages)
						.add_attributes(response.attributes)
						.add_events(response.events),
					None => IbcReceiveResponse::without_ack()
						.add_submessages(response.messages)
						.add_attributes(response.attributes)
						.add_events(response.events),
				}),
...
pub enum ExecutorError {
	...
	/// The requrested deallocation size is too big and would overflow the memory (must be
	/// impossible).
...
pub mod constants {
	...
	/// Max key length for `db_write/db_read/db_remove/db_scan` (when VM reads the key argument from
	/// Wasm memory)
	...
	/// This is an arbitrary value, for performance / memory contraints. If you need to verify
	/// larger messages, let us know.
	pub const MAX_LENGTH_ED25519_MESSAGE: usize = 128 * 1024;
	/// Max number of batch Ed25519 messages / signatures / `public_keys`.
	/// This is an arbitrary value, for performance / memory contraints. If you need to batch-verify
	/// a larger number of signatures, let us know.
...
/// Allow for untyped marshalling to specify a limit while extracting the bytes from a contract
/// memory.
...
/// Allocate memory in the contract and write the type `T` serialized in JSON at the newly allocated
/// space.
///
/// # Arguments
///
...
///
/// * `vm` - the virtual machine.
/// * `pointer` - the region pointer from which we will read and deserialized a JSON payload of type
///   `T`.
///
...
/// Execute a generic contract export (`instantiate`, `execute`, `migrate` etc...), providing the
/// custom raw `message` input.
///
/// # Arguments
///
/// * `vm` - the virtual machine.
/// * `message` - the contract message passed to the export, usually specific to the contract
///   (`InstantiateMsg`, `ExecuteMsg` etc...).
///
...
where
	...
	VmErrorOf<V>:
		From<ReadableMemoryErrorOf<V>> + From<WritableMemoryErrorOf<V>> + From<ExecutorError>,
{
	cosmwasm_call(vm, &serde_json::to_vec(message).map_err(|_| ExecutorError::FailedToSerialize)?)
}
