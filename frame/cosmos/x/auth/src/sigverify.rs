...
pub fn ecdsa_verify_prehashed(signature: &[u8], message_hash: &[u8; 32], public_key: &[u8]) -> bool {
	...
	match signature.len() {
		...
		65 => ecdsa::Signature::try_from(signature)
			.map_or(false, |sig| sp_io::crypto::ecdsa_verify_prehashed(&sig, message_hash, &pub_key)),
		_ => false,
	}
}
