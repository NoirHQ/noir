...
pub type BuiltinFunctionWithContext<T> = BuiltinFunction<InvokeContext<'static, T>>;
...
macro_rules! declare_process_instruction {
    ($process_instruction:ident, $cu_to_consume:expr, |$invoke_context:ident| $inner:tt) => {
        $crate::declare_builtin_function!(
            $process_instruction,
            fn rust(
                invoke_context: &mut $crate::invoke_context::InvokeContext<T>,
                _arg0: u64,
                _arg1: u64,
                _arg2: u64,
                _arg3: u64,
                ...
                fn process_instruction_inner<T: $crate::Config>(
                    $invoke_context: &mut $crate::invoke_context::InvokeContext<T>,
...
impl<T: crate::Config> ContextObject for InvokeContext<'_, T> {
	fn trace(&mut self, state: [u64; 12]) {
		self.syscall_context.last_mut().unwrap().as_mut().unwrap().trace_log.push(state);
	}
...
impl BpfAllocator {
	...
	pub fn alloc(&mut self, layout: Layout) -> Result<u64, AllocErr> {
		...
		if self.pos.saturating_add(bytes_to_align).saturating_add(layout.size() as u64) <= self.len
		{
...
pub struct InvokeContext<'a, T: crate::Config> {
	/// Information about the currently executing transaction.
	pub transaction_context: &'a mut TransactionContext<T>,
	// The local program cache for the transaction batch.
	pub program_cache_for_tx_batch: &'a mut ProgramCacheForTxBatch<T>,
...
impl<'a, T: crate::Config> InvokeContext<'a, T> {
	#[allow(clippy::too_many_arguments)]
	pub fn new(
		transaction_context: &'a mut TransactionContext<T>,
		program_cache_for_tx_batch: &'a mut ProgramCacheForTxBatch<T>,
	...
	pub fn push(&mut self) -> Result<(), InstructionError> {
		let instruction_context =
			self.transaction_context.get_instruction_context_at_index_in_trace(
				self.transaction_context.get_instruction_trace_length(),
			)?;
		let program_id = instruction_context
			.get_last_program_key(self.transaction_context)
			.map_err(|_| InstructionError::UnsupportedProgramId)?;
		if self.transaction_context.get_instruction_context_stack_height() != 0 {
			let contains = (0..self.transaction_context.get_instruction_context_stack_height())
				.any(|level| {
	...
	pub fn prepare_instruction(
		...
		for (instruction_account_index, account_meta) in instruction.accounts.iter().enumerate() {
			...
			if let Some(duplicate_index) =
				deduplicated_instruction_accounts.iter().position(|instruction_account| {
		...
		for instruction_account in deduplicated_instruction_accounts.iter() {
			...
			if instruction_account.is_writable && !borrowed_account.is_writable() {
				ic_msg!(self, "{}'s writable privilege escalated", borrowed_account.get_key(),);
			...
			{
				ic_msg!(self, "{}'s signer privilege escalated", borrowed_account.get_key());
				return Err(InstructionError::PrivilegeEscalation);
			}
		}
		...
		Ok((instruction_accounts, vec![borrowed_program_account.get_index_in_transaction()]))
	}
	...
	fn process_executable_chain(
		...
		let mut vm = EbpfVm::new(
			self.program_cache_for_tx_batch.environments.program_runtime_v2.clone(),
			&SBPFVersion::V1,
			// Removes lifetime tracking
			unsafe { core::mem::transmute::<&mut InvokeContext<T>, &mut InvokeContext<T>>(self) },
	...
	*/
	...
	pub fn set_syscall_context(
		...
		*self.syscall_context.last_mut().ok_or(InstructionError::CallDepth)? =
			Some(syscall_context);
		Ok(())
	}
...
macro_rules! with_mock_invoke_context {
	(
		...
		let mut $invoke_context = <InvokeContext<Test>>::new(
...
pub fn mock_process_instruction<
	F: FnMut(&mut InvokeContext<Test>),
	G: FnMut(&mut InvokeContext<Test>),
>(
	loader_id: &Pubkey,
	mut program_indices: Vec<IndexOfAccount>,
	instruction_data: &[u8],
	mut transaction_accounts: Vec<TransactionAccount<Test>>,
	instruction_account_metas: Vec<AccountMeta>,
	expected_result: Result<(), InstructionError>,
	builtin_function: BuiltinFunctionWithContext<Test>,
	mut pre_adjustments: F,
	mut post_adjustments: G,
) -> Vec<AccountSharedData<Test>> {
	...
	let mut program_cache_for_tx_batch = ProgramCacheForTxBatch::default();
	program_cache_for_tx_batch
		.replenish(*loader_id, Arc::new(ProgramCacheEntry::new_builtin(0, 0, builtin_function)));
...
mod tests {
	...
	declare_process_instruction!(MockBuiltin, MOCK_BUILTIN_COMPUTE_UNIT_COST, |invoke_context| {
	...
	fn test_instruction_stack_height() {
		let one_more_than_max_depth =
			ComputeBudget::default().max_instruction_stack_depth.saturating_add(1);
		...
		for index in 0..one_more_than_max_depth {
			...
			transaction_accounts.push((
				...
				<AccountSharedData<Test>>::new(index as u64, 1, invoke_stack.get(index).unwrap()),
			));
			instruction_accounts.push(InstructionAccount {
	...
	fn test_max_instruction_trace_length() {
		...
		let mut transaction_context =
			<TransactionContext<Test>>::new(Vec::new(), Rent::default(), 1, MAX_INSTRUCTIONS);
	...
	fn test_process_instruction() {
		...
		let owned_account = <AccountSharedData<Test>>::new(42, 1, &callee_program_id);
		let not_owned_account =
			<AccountSharedData<Test>>::new(84, 1, &solana_sdk::pubkey::new_rand());
		let readonly_account =
			<AccountSharedData<Test>>::new(168, 1, &solana_sdk::pubkey::new_rand());
		let loader_account = <AccountSharedData<Test>>::new(0, 1, &native_loader::id());
		let mut program_account = <AccountSharedData<Test>>::new(1, 1, &native_loader::id());
		...
		let cases = vec![
			...
			(MockInstruction::NoopFail, Err(InstructionError::GenericError)),
			(MockInstruction::ModifyOwned, Ok(())),
			(MockInstruction::ModifyNotOwned, Err(InstructionError::ExternalAccountDataModified)),
			(MockInstruction::ModifyReadonly, Err(InstructionError::ReadonlyDataModified)),
			(MockInstruction::UnbalancedPush, Err(InstructionError::UnbalancedInstruction)),
			(MockInstruction::UnbalancedPop, Err(InstructionError::UnbalancedInstruction)),
		];
		for case in cases {
			invoke_context
				.transaction_context
		...
		for expected_result in expected_results {
			...
			let (inner_instruction_accounts, program_indices) =
				invoke_context.prepare_instruction(&inner_instruction, &[]).unwrap();
	...
	fn test_invoke_context_compute_budget() {
		let transaction_accounts =
			vec![(solana_sdk::pubkey::new_rand(), <AccountSharedData<Test>>::default())];
	...
	fn test_process_instruction_accounts_resize_delta() {
		...
		let user_account =
			<AccountSharedData<Test>>::new(100, user_account_data_len as usize, &program_key);
		let dummy_account = <AccountSharedData<Test>>::new(10, 0, &program_key);
		let mut program_account = <AccountSharedData<Test>>::new(500, 500, &native_loader::id());
		program_account.set_executable(true);
		let transaction_accounts = vec![
		...
		// must succeed
		{
			...
			assert!(result.is_ok());
			assert_eq!(
				invoke_context.transaction_context.accounts_resize_delta().unwrap(),
				resize_delta
			);
		}
