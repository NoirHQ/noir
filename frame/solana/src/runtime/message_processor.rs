...
use solana_sdk::{
	account::WritableAccount, message::SanitizedMessage, precompiles::is_precompile,
	saturating_add_assign, sysvar::instructions, transaction::TransactionError,
};
...
impl MessageProcessor {
	...
	pub fn process_message<T: Config>(
		...
		{
			let is_precompile =
				is_precompile(program_id, |id| invoke_context.get_feature_set().is_active(id));

			// Fixup the special instructions key if present
			// before the account pre-values are taken care of
			if let Some(account_index) =
				invoke_context.transaction_context.find_index_of_account(&instructions::id())
			{
			...
			let result = if is_precompile {
				...
				*accumulated_consumed_units =
				...
				saturating_add_assign!(
					execute_timings.execute_accessories.process_instructions.total_us,
					time
				);
				result
			};
...
mod tests {
	...
	fn test_process_message_readonly_handling() {
		...
		let accounts = vec![
			(writable_pubkey, AccountSharedData::new(100, 1, &mock_system_program_id)),
			(readonly_pubkey, AccountSharedData::new(0, 1, &mock_system_program_id)),
			(mock_system_program_id, create_loadable_account_for_test("mock_system_program")),
		];
		...
		let account_keys = (0..transaction_context.get_number_of_accounts())
			.map(|index| *transaction_context.get_key_of_account_at_index(index).unwrap())
			.collect::<Vec<_>>();
		...
		assert!(result.is_ok());
		assert_eq!(transaction_context.get_account_at_index(0).unwrap().borrow().lamports(), 100);
		assert_eq!(transaction_context.get_account_at_index(1).unwrap().borrow().lamports(), 0);
		...
		assert_eq!(
			result,
			Err(TransactionError::InstructionError(0, InstructionError::ReadonlyLamportChange))
		);
		...
		assert_eq!(
			result,
			Err(TransactionError::InstructionError(0, InstructionError::ReadonlyDataModified))
		);
	}
	...
	fn test_process_message_duplicate_accounts() {
		...
		let accounts = vec![
			(solana_sdk::pubkey::new_rand(), AccountSharedData::new(100, 1, &mock_program_id)),
			(solana_sdk::pubkey::new_rand(), AccountSharedData::new(0, 1, &mock_program_id)),
			(mock_program_id, create_loadable_account_for_test("mock_system_program")),
		];
		let mut transaction_context = TransactionContext::new(accounts, Rent::default(), 1, 3);
		let program_indices = vec![vec![2]];
		let mut program_cache_for_tx_batch = ProgramCacheForTxBatch::default();
		...
		let account_metas = vec![
			AccountMeta::new(*transaction_context.get_key_of_account_at_index(0).unwrap(), true),
			AccountMeta::new(*transaction_context.get_key_of_account_at_index(1).unwrap(), false),
			AccountMeta::new(*transaction_context.get_key_of_account_at_index(0).unwrap(), false),
		];
		...
		assert_eq!(
			result,
			Err(TransactionError::InstructionError(0, InstructionError::AccountBorrowFailed))
		);
		...
		let message = new_sanitized_message(Message::new(
			&[Instruction::new_with_bincode(
				mock_program_id,
				&MockSystemInstruction::DoWork { lamports: 10, data: 42 },
				account_metas,
			)],
			Some(transaction_context.get_key_of_account_at_index(0).unwrap()),
		));
		...
		assert!(result.is_ok());
		assert_eq!(transaction_context.get_account_at_index(0).unwrap().borrow().lamports(), 80);
		assert_eq!(transaction_context.get_account_at_index(1).unwrap().borrow().lamports(), 20);
		assert_eq!(transaction_context.get_account_at_index(0).unwrap().borrow().data(), &vec![42]);
	}
	...
	fn test_precompile() {
		...
		let accounts = vec![
			(Pubkey::new_unique(), AccountSharedData::new(1, 0, &system_program::id())),
		...
		assert_eq!(
			result,
			Err(TransactionError::InstructionError(1, InstructionError::Custom(0xbabb1e)))
		);
