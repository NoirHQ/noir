...
use solana_program_runtime::{
	dummy::VoteAccountsHashMap, log_collector::LogCollector, sysvar_cache::SysvarCache,
...
pub struct LoadAndExecuteSanitizedTransactionOutput<T: Config> {
	...
	pub loaded_transaction: TransactionLoadResult<T>,
}
...
#[derive(Default)]
pub struct TransactionProcessingConfig<'a, T: Config> {
	/// Encapsulates overridden accounts, typically used for transaction
	/// simulation.
	pub account_overrides: Option<&'a AccountOverrides<T>>,
...
impl<T: Config> TransactionProcessor<T> {
	fn load_and_execute_sanatized_transaction<CB: TransactionProcessingCallback<T>>(
		...
		check_result: TransactionCheckResult<T>,
		environment: &TransactionProcessingEnvironment,
		config: &TransactionProcessingConfig<T>,
	) -> LoadAndExecuteSanitizedTransactionOutput<T> {
		// Initialize metrics.
	...
	fn validate_fee<CB: TransactionProcessingCallback<T>>(
		&self,
		callbacks: &CB,
	...
	fn validate_transaction_fee_payer<CB: TransactionProcessingCallback<T>>(
		&self,
		callbacks: &CB,
		account_overrides: Option<&AccountOverrides<T>>,
		message: &SanitizedMessage,
		checked_details: CheckedTransactionDetails<T>,
		feature_set: &FeatureSet,
		fee_structure: &FeeStructure,
		rent_collector: &RentCollector,
		error_counters: &mut TransactionErrorMetrics,
	...
	fn execute_loaded_transaction(
		&self,
		tx: &SanitizedTransaction,
		loaded_transaction: &mut LoadedTransaction<T>,
		execute_timings: &mut ExecuteTimings,
		error_metrics: &mut TransactionErrorMetrics,
		program_cache_for_tx_batch: &mut ProgramCacheForTxBatch<T>,
		environment: &TransactionProcessingEnvironment,
		config: &TransactionProcessingConfig<T>,
	) -> TransactionExecutionResult<T> {
		let transaction_accounts = core::mem::take(&mut loaded_transaction.accounts);

		fn transaction_accounts_lamports_sum<T: Config>(
			accounts: &[(Pubkey, AccountSharedData<T>)],
			message: &SanitizedMessage,
		) -> Option<u128> {
