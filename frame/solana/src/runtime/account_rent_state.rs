...
impl RentState {
	...
	pub fn from_account<T: Config>(account: &AccountSharedData<T>, rent: &Rent) -> Self {
		if account.lamports() == 0 {
			Self::Uninitialized
		} else if rent.is_exempt(account.lamports(), account.data().len()) {
			Self::RentExempt
		} else {
			Self::RentPaying { data_size: account.data().len(), lamports: account.lamports() }
		}
	}
		...
		match self {
			...
			Self::RentPaying { data_size: post_data_size, lamports: post_lamports } => {
				match pre_rent_state {
					...
					Self::RentPaying { data_size: pre_data_size, lamports: pre_lamports } => {
	...
	pub(crate) fn check_rent_state<T: Config>(
		...
		if let Some((pre_rent_state, post_rent_state)) = pre_rent_state.zip(post_rent_state) {
			...
			Self::check_rent_state_with_account(
				...
				&transaction_context.get_account_at_index(index).expect(expect_msg).borrow(),
				index,
			)?;
		}
		Ok(())
	...
	pub(super) fn check_rent_state_with_account<T: Config>(
		pre_rent_state: &Self,
		post_rent_state: &Self,
		address: &Pubkey,
		account_state: &AccountSharedData<T>,
		...
		{
			debug!("Account {} not rent exempt, state {:?}", address, account_state,);
	...
	fn submit_rent_state_metrics(_pre_rent_state: &Self, _post_rent_state: &Self) {
		/*
...
mod tests {
	...
	fn test_from_account() {
		...
		assert_eq!(RentState::from_account(&rent_exempt_account, &rent), RentState::RentExempt);
	...
	fn test_transition_allowed_from() {
		...
		assert!(post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 0, lamports: 1 }));
		...
		assert!(post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 0, lamports: 1 }));
		let post_rent_state = RentState::RentPaying { data_size: 2, lamports: 5 };
		assert!(!post_rent_state.transition_allowed_from(&RentState::Uninitialized));
		assert!(!post_rent_state.transition_allowed_from(&RentState::RentExempt));
		assert!(!post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 3, lamports: 5 }));
		assert!(!post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 1, lamports: 5 }));
		// Transition is always allowed if there is no account data resize or
		// change in account's lamports.
		assert!(post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 2, lamports: 5 }));
		// Transition is always allowed if there is no account data resize and
		// account's lamports is reduced.
		assert!(post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 2, lamports: 7 }));
		// Transition is not allowed if the account is credited with more
		// lamports and remains rent-paying.
		assert!(!post_rent_state
			.transition_allowed_from(&RentState::RentPaying { data_size: 2, lamports: 3 }),);
	}

	#[test]
	fn test_check_rent_state_with_account() {
		let pre_rent_state = RentState::RentPaying { data_size: 2, lamports: 3 };
		...
		assert_eq!(
			result.err(),
			Some(TransactionError::InsufficientFundsForRent { account_index: account_index as u8 })
		);
