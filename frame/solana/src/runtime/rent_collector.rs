...
impl RentCollector {
	pub fn new(
		...
		Self { epoch, epoch_schedule, slots_per_year, rent }
	}

	pub fn clone_with_epoch(&self, epoch: Epoch) -> Self {
		Self { epoch, ..self.clone() }
	}
	...
	pub fn get_rent_due(
		...
		if self.rent.is_exempt(lamports, data_len) {
			...
			let slots_elapsed: u64 = (account_rent_epoch..=self.epoch)
				.map(|epoch| self.epoch_schedule.get_slots_in_epoch(epoch.saturating_add(1)))
				.sum();
	...
	pub fn collect_from_existing_account<T: Config>(
		&self,
		address: &Pubkey,
		...
		match self.calculate_rent_result(address, account) {
			...
			RentResult::CollectRent { new_rent_epoch, rent_due } =>
				match account.lamports().checked_sub(rent_due) {
					...
					Some(lamports) => {
						account.set_lamports(lamports.into());
						account.set_rent_epoch(new_rent_epoch);
						CollectedInfo { rent_amount: rent_due, account_data_len_reclaimed: 0u64 }
					},
				},
		}
	}
	...
	fn calculate_rent_result(
		...
		match self.get_rent_due(account.lamports(), account.data().len(), account.rent_epoch()) {
			...
			RentDue::Paying(rent_due) =>
				RentResult::CollectRent { new_rent_epoch: self.epoch.saturating_add(1), rent_due },
		}
	}
}
...
mod tests {
	...
	use solana_sdk::{account::Account, sysvar};
	...
	fn test_calculate_rent_result() {
		...
		for (rent_epoch, rent_due_expected) in [(2, 2), (3, 5)] {
...

			{
				...
				let mut account_expected = account.clone();
				account_expected.set_lamports(account.get_lamports() - rent_due_expected);
		...
		account.set_lamports(1_000_000.into());
	...
	fn test_collect_from_account_created_and_existing() {
		...
		assert_eq!(created_account.lamports() + collected.rent_amount, old_lamports);
		...
		assert_eq!(existing_account.lamports() + collected.rent_amount, old_lamports);
	...
	fn test_rent_exempt_temporal_escape() {
		for pass in 0..2 {
			...
			if pass == 0 {
				account.set_lamports(huge_lamports.into());
			...
			// In a real validator, it is not legal to reduce an account's lamports such that the
			// account becomes rent paying. So, pass == 0 above tests the case of rent that is
			// exempt. pass == 1 tests the case where we are rent paying.
			account.set_lamports(tiny_lamports.into());

			// ... and trigger another rent collection on the same epoch and check that rent is
			// working
			let collected = rent_collector.collect_from_existing_account(&pubkey, &mut account);
	...
	fn test_collect_cleans_up_account() {
		...
		assert_eq!(collected.account_data_len_reclaimed, account_data_len as u64);
