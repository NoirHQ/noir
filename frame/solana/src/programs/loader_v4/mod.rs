...
use crate::runtime::{
	...
	loaded_programs::{
		LoadProgramMetrics, ProgramCacheEntry, ProgramCacheEntryType, DELAY_VISIBILITY_SLOT_OFFSET,
	},
...
use solana_program_runtime::{
	dummy::Measure, ic_logger_msg, log_collector::LogCollector, stable_log,
};
use solana_rbpf::{
...
pub fn get_state(data: &[u8]) -> Result<&LoaderV4State, InstructionError> {
	unsafe {
		let data = data
			...
			.try_into()
			.unwrap();
		Ok(core::mem::transmute::<&[u8; LoaderV4State::program_data_offset()], &LoaderV4State>(
			data,
		))
	}
}
...
pub fn create_program_runtime_environment_v2<'a, T: crate::Config>(
	compute_budget: &ComputeBudget,
	debugging_features: bool,
) -> BuiltinProgram<InvokeContext<'a, T>> {
	let config = Config {
...
pub fn create_vm<'a, 'b, T: crate::Config>(
	invoke_context: &'a mut InvokeContext<'b, T>,
	program: &'a Executable<InvokeContext<'b, T>>,
) -> Result<EbpfVm<'a, InvokeContext<'b, T>>, Box<dyn core::error::Error>> {
...
fn execute<'a, 'b: 'a, T: crate::Config>(
	invoke_context: &'a mut InvokeContext<'b, T>,
	executable: &'a Executable<InvokeContext<'static, T>>,
) -> Result<(), Box<dyn core::error::Error>> {
	// We dropped the lifetime tracking in the Executor by setting it to 'static,
	// thus we need to reintroduce the correct lifetime of InvokeContext here again.
	let executable = unsafe {
		core::mem::transmute::<
			&'a Executable<InvokeContext<'static, T>>,
			&'a Executable<InvokeContext<'b, T>>,
		>(executable)
	};
	let log_collector = invoke_context.get_log_collector();
...
fn check_program_account<T: crate::Config>(
	log_collector: &Option<Rc<RefCell<LogCollector>>>,
	instruction_context: &InstructionContext<T>,
	program: &BorrowedAccount<T>,
...
pub fn process_instruction_write<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
	offset: u32,
	bytes: Vec<u8>,
	...
	let state =
		check_program_account(&log_collector, instruction_context, &program, authority_address)?;
...
pub fn process_instruction_truncate<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
	new_size: u32,
) -> Result<(), InstructionError> {
	let log_collector = invoke_context.get_log_collector();
...
pub fn process_instruction_deploy<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
) -> Result<(), InstructionError> {
	let log_collector = invoke_context.get_log_collector();
	let transaction_context = &invoke_context.transaction_context;
	...
	let authority_address = instruction_context
		...
		.and_then(|index| transaction_context.get_key_of_account_at_index(index))?;
	let source_program =
		instruction_context.try_borrow_instruction_account(transaction_context, 2).ok();
	let state =
		check_program_account(&log_collector, instruction_context, &program, authority_address)?;
	...
	if state.slot != 0 && state.slot.saturating_add(DEPLOYMENT_COOLDOWN_IN_SLOTS) > current_slot {
		ic_logger_msg!(log_collector, "Program was deployed recently, cooldown still in effect");
		return Err(InstructionError::InvalidArgument);
	}
	if !matches!(state.status, LoaderV4Status::Retracted) {
	...
	let environments = invoke_context.get_environments_for_slot(effective_slot).map_err(|err| {
	...
	if let Some(old_entry) = invoke_context.program_cache_for_tx_batch.find(program.get_key()) {
		executor
			.tx_usage_counter
			.store(old_entry.tx_usage_counter.load(Ordering::Relaxed), Ordering::Relaxed);
		executor
			.ix_usage_counter
			.store(old_entry.ix_usage_counter.load(Ordering::Relaxed), Ordering::Relaxed);
	}
	invoke_context
...
pub fn process_instruction_retract<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
) -> Result<(), InstructionError> {
	let log_collector = invoke_context.get_log_collector();
	let transaction_context = &invoke_context.transaction_context;
...
pub fn process_instruction_transfer_authority<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
) -> Result<(), InstructionError> {
	let log_collector = invoke_context.get_log_collector();
	let transaction_context = &invoke_context.transaction_context;
...
pub fn process_instruction_inner<T: crate::Config>(
	invoke_context: &mut InvokeContext<T>,
) -> Result<u64, Box<dyn core::error::Error>> {
	let log_collector = invoke_context.get_log_collector();
	let transaction_context = &invoke_context.transaction_context;
	...
	if loader_v4::check_id(program_id) {
		...
		//);
		drop(program);
		loaded_program.ix_usage_counter.fetch_add(1, Ordering::Relaxed);
...
mod tests {
	...
	use crate::{
		mock::Test,
		runtime::{
			account::{create_account_shared_data_for_test, ReadableAccount},
	...
	type InvokeContext<'a> = crate::runtime::invoke_context::InvokeContext<'a, Test>;
			...
			if loader_v4::check_id(owner) {
					...
					account.data().get(LoaderV4State::program_data_offset()..)
				{
					if let Ok(loaded_program) = ProgramCacheEntry::new(
						...
						invoke_context.program_cache_for_tx_batch.set_slot_for_tests(0);
						invoke_context
							.program_cache_for_tx_batch
	...
	fn process_instruction(
		...
		let instruction_accounts = instruction_accounts
			.iter()
			.map(|(index_in_transaction, is_signer, is_writable)| AccountMeta {
		...
		mock_process_instruction(
			...
			Entrypoint::vm,
			|invoke_context| {
				invoke_context.program_cache_for_tx_batch.environments.program_runtime_v2 =
	...
	fn load_program_account_from_elf(
		...
		let path = Path::new("src/programs/loader_v4/test_elfs/out/")
			.join(path)
			.with_extension("so");
	...
	fn clock(slot: Slot) -> AccountSharedData {
		let clock = clock::Clock { slot, ..clock::Clock::default() };
	...
	fn test_loader_instruction_general_errors(instruction: LoaderV4Instruction) {
		...
		let transaction_accounts = vec![
			...
			(authority_address, AccountSharedData::new(0, 0, &Pubkey::new_unique())),
			(
				Pubkey::new_unique(),
				load_program_account_from_elf(
					authority_address,
			...
			(clock::id(), create_account_shared_data_for_test(&clock::Clock::default())),
			(rent::id(), create_account_shared_data_for_test(&rent::Rent::default())),
		];
	...
	fn test_loader_instruction_write() {
		...
		process_instruction(
			vec![],
			&bincode::serialize(&LoaderV4Instruction::Write { offset: 2, bytes: vec![8, 8, 8, 8] })
				.unwrap(),
			transaction_accounts.clone(),
		...
		process_instruction(
			vec![],
			&bincode::serialize(&LoaderV4Instruction::Write { offset: 2, bytes: Vec::new() })
				.unwrap(),
			transaction_accounts.clone(),
			&[(0, false, true), (1, true, false)],
			Ok(()),
		...
		process_instruction(
			vec![],
			&bincode::serialize(&LoaderV4Instruction::Write { offset: 8, bytes: vec![8, 8, 8, 8] })
				.unwrap(),
			transaction_accounts.clone(),
	...
	fn test_loader_instruction_truncate() {
		...
		let mut transaction_accounts = vec![
			...
			(authority_address, AccountSharedData::new(0, 0, &Pubkey::new_unique())),
			(Pubkey::new_unique(), AccountSharedData::new(0, 0, &loader_v4::id())),
			(Pubkey::new_unique(), AccountSharedData::new(40000000, 0, &loader_v4::id())),
			(
				Pubkey::new_unique(),
				load_program_account_from_elf(
					authority_address,
		...
		assert_eq!(accounts[0].data().len(), transaction_accounts[0].1.data().len(),);
		assert_eq!(accounts[2].lamports(), transaction_accounts[2].1.lamports());
		let lamports = transaction_accounts[4].1.lamports();
		transaction_accounts[0].1.set_lamports(lamports.into());
		...
		assert_eq!(accounts[3].data().len(), transaction_accounts[0].1.data().len(),);
		...
		assert_eq!(accounts[0].data().len(), transaction_accounts[4].1.data().len(),);
		...
		assert_eq!(accounts[4].data().len(), transaction_accounts[0].1.data().len(),);
		assert_eq!(
			accounts[2].lamports(),
			transaction_accounts[2].1.lamports().saturating_add(
				transaction_accounts[4].1.lamports().saturating_sub(accounts[4].lamports())
			),
		);
		...
		assert_eq!(
			accounts[2].lamports(),
			transaction_accounts[2].1.lamports().saturating_add(
				transaction_accounts[0].1.lamports().saturating_sub(accounts[0].lamports())
			),
		);
	...
	fn test_loader_instruction_deploy() {
		...
		transaction_accounts[0].1 = accounts[0].clone();
		assert_eq!(accounts[0].data().len(), transaction_accounts[2].1.data().len(),);
		assert_eq!(accounts[2].data().len(), 0,);
		assert_eq!(
			accounts[2].lamports(),
			transaction_accounts[2].1.lamports().saturating_sub(
				accounts[0].lamports().saturating_sub(transaction_accounts[0].1.lamports())
			),
		);
	...
	fn test_loader_instruction_transfer_authority() {
		...
		let transaction_accounts = vec![
			...
			(Pubkey::new_unique(), AccountSharedData::new(0, 0, &loader_v4::id())),
			(authority_address, AccountSharedData::new(0, 0, &Pubkey::new_unique())),
			(Pubkey::new_unique(), AccountSharedData::new(0, 0, &Pubkey::new_unique())),
			(clock::id(), create_account_shared_data_for_test(&clock::Clock::default())),
			(rent::id(), create_account_shared_data_for_test(&rent::Rent::default())),
		];
	...
	fn test_execute_program() {
		...
		let transaction_accounts = vec![
			...
			(Pubkey::new_unique(), AccountSharedData::new(10000000, 32, &program_address)),
			(Pubkey::new_unique(), AccountSharedData::new(0, 0, &loader_v4::id())),
			(
				Pubkey::new_unique(),
				load_program_account_from_elf(
