...
use crate::{
	runtime::{
		declare_process_instruction, ic_msg,
...
impl Address {
	...
	fn create<T: Config>(
		...
		Ok(Self { address: *address, base })
	}
}

fn allocate<T: Config>(
	account: &mut BorrowedAccount<T>,
	address: &Address,
	space: u64,
...
fn allocate_and_assign<T: Config>(
	to: &mut BorrowedAccount<T>,
	to_address: &Address,
	space: u64,
	owner: &Pubkey,
	signers: &HashSet<Pubkey>,
...
fn transfer_with_seed<T: Config>(
	from_account_index: IndexOfAccount,
...
declare_process_instruction!(Entrypoint, DEFAULT_COMPUTE_UNITS, |invoke_context| {
	...
	match instruction {
		...
		SystemInstruction::Transfer { lamports } => {
			instruction_context.check_number_of_instruction_accounts(2)?;
			transfer(0, 1, lamports, invoke_context, transaction_context, instruction_context)
		},
		...
		SystemInstruction::AdvanceNonceAccount => {
			...
			if recent_blockhashes.is_empty() {
				ic_msg!(invoke_context, "Advance nonce account: recent blockhash list is empty",);
		...
		SystemInstruction::InitializeNonceAccount(authorized) => {
			...
			if recent_blockhashes.is_empty() {
				ic_msg!(invoke_context, "Initialize nonce account: recent blockhash list is empty",);
				return Err(SystemError::NonceNoRecentBlockhashes.into());
			}
		...
		SystemInstruction::AllocateWithSeed { base, seed, space, owner } => {
			...
			allocate_and_assign(&mut account, &address, space, &owner, &signers, invoke_context)
		},
...
mod tests {
	...
	use crate::{
		mock::Test,
		programs::system::*,
		runtime::{invoke_context::mock_process_instruction, nonce_account},
	...
	use solana_sdk::{
		account::{
			self, create_account_shared_data_with_fields, Account, ReadableAccount,
	...
	pub fn to_account<S: Sysvar>(sysvar: &S, account: &mut AccountSharedData) -> Option<()> {
	...
	fn create_default_rent_account() -> AccountSharedData {
		account::create_account_shared_data_for_test(&Rent::free()).into()
	}
	...
	fn test_create_account_with_seed_missing_sig() {
		...
		let accounts = process_instruction(
			...
			vec![(from, from_account), (to, to_account)],
			vec![
				AccountMeta { pubkey: from, is_signer: true, is_writable: false },
				AccountMeta { pubkey: to, is_signer: false, is_writable: false },
			],
	...
	fn test_create_already_in_use() {
		...
		let accounts = process_instruction(
			...
			vec![
				AccountMeta { pubkey: from, is_signer: true, is_writable: false },
				AccountMeta { pubkey: owned_key, is_signer: true, is_writable: false },
			],
	...
	fn test_create_unsigned() {
		...
		process_instruction(
			...
			vec![
				AccountMeta { pubkey: from, is_signer: false, is_writable: false },
				AccountMeta { pubkey: owned_key, is_signer: false, is_writable: false },
			],
			Err(InstructionError::MissingRequiredSignature),
		);
	...
	fn test_create_data_populated() {
		...
		let populated_account =
			AccountSharedData::from(Account { data: vec![0, 1, 2, 3], ..Account::default() });
		...
		process_instruction(
			...
			vec![
				AccountMeta { pubkey: from, is_signer: true, is_writable: false },
				AccountMeta { pubkey: populated_key, is_signer: true, is_writable: false },
			],
			Err(SystemError::AccountAlreadyInUse.into()),
		);
	}
	...
	fn test_create_from_account_is_nonce_fail() {
		...
		process_instruction(
			...
			vec![
				AccountMeta { pubkey: nonce, is_signer: true, is_writable: false },
				AccountMeta { pubkey: new, is_signer: true, is_writable: true },
			],
	...
	fn test_assign() {
		...
		process_instruction(
			&bincode::serialize(&SystemInstruction::Assign { owner: system_program::id() })
				.unwrap(),
			vec![(pubkey, account.clone())],
			vec![AccountMeta { pubkey, is_signer: false, is_writable: true }],
			Ok(()),
		);
		...
		process_instruction(
			&bincode::serialize(&SystemInstruction::Assign { owner: new_owner }).unwrap(),
			vec![(pubkey, account.clone())],
			vec![AccountMeta { pubkey, is_signer: true, is_writable: true }],
			Ok(()),
		);

		// assign to sysvar instead of system_program
		process_instruction(
			&bincode::serialize(&SystemInstruction::Assign { owner: sysvar::id() }).unwrap(),
			vec![(pubkey, account)],
			vec![AccountMeta { pubkey, is_signer: true, is_writable: true }],
			Ok(()),
		);
	...
	fn test_process_bogus_instruction() {
		...
		let instruction = SystemInstruction::Assign { owner: Pubkey::new_unique() };
		...
		process_instruction(
			...
			vec![AccountMeta { pubkey: from, is_signer: true, is_writable: false }],
			Err(InstructionError::NotEnoughAccountKeys),
		);
	}
	...
	fn test_transfer_lamports() {
		...
		let accounts = process_instruction(
			...
			transaction_accounts,
			vec![
				AccountMeta { pubkey: from, is_signer: false, is_writable: true },
				AccountMeta { pubkey: to, is_signer: false, is_writable: true },
			],
			Err(InstructionError::MissingRequiredSignature),
		);
	...
	fn test_transfer_lamports_from_nonce_account_fail() {
		...
		.unwrap();
		assert_eq!(get_system_account_kind(&from_account), Some(SystemAccountKind::Nonce));
		let to = Pubkey::from([3; 32]);
		let to_account = AccountSharedData::new(1, 0, &to); // account owner should not matter
	...
	fn test_process_nonce_ix_ok() {
		...
		let accounts = process_instruction(
			...
			vec![
				AccountMeta { pubkey: nonce_address, is_signer: true, is_writable: true },
				AccountMeta { pubkey: blockhash_id, is_signer: false, is_writable: false },
				AccountMeta { pubkey: sysvar::rent::id(), is_signer: false, is_writable: false },
			],
			Ok(()),
		);
		...
		mock_process_instruction(
			...
			Ok(()),
			Entrypoint::vm,
			|invoke_context: &mut InvokeContext<Test>| {
	...
	fn test_process_withdraw_ix_only_nonce_acc_fail() {
		...
		process_instruction(
			...
			vec![AccountMeta { pubkey: nonce_address, is_signer: true, is_writable: true }],
			Err(InstructionError::NotEnoughAccountKeys),
		);
	}
	...
	fn test_get_system_account_kind_system_ok() {
		...
		assert_eq!(get_system_account_kind(&system_account), Some(SystemAccountKind::System));
	}
	...
	fn test_get_system_account_kind_nonce_ok() {
		...
		.unwrap();
		assert_eq!(get_system_account_kind(&nonce_account), Some(SystemAccountKind::Nonce));
	}

	#[test]
	fn test_get_system_account_kind_uninitialized_nonce_account_fail() {
		assert_eq!(
			get_system_account_kind(&nonce_account::create_account::<Test>(42).borrow()),
			None
		);
	}
	...
	fn test_nonce_account_upgrade_check_owner() {
		...
		let accounts = process_instruction(
			...
			vec![AccountMeta { pubkey: nonce_address, is_signer: false, is_writable: true }],
	...
	fn new_nonce_account(versions: NonceVersions) -> AccountSharedData {
		...
		.unwrap();
		assert_eq!(nonce_account.deserialize_data::<NonceVersions>().unwrap(), versions);
		nonce_account
	}
	...
	fn test_nonce_account_upgrade() {
		...
		let data = NonceData { durable_nonce, ..data };
	...
	fn test_assign_native_loader_and_transfer() {
		for size in [0, 10] {
			...
			let accounts = process_instruction(
				&bincode::serialize(&SystemInstruction::Transfer { lamports: 50 }).unwrap(),
				vec![
					(pubkey2, AccountSharedData::new(100, 0, &system_program::id())),
					(pubkey, accounts[0].clone()),
				],
				vec![
					AccountMeta { pubkey: pubkey2, is_signer: true, is_writable: true },
