...
pub fn advance_nonce_account<T: Config>(
...
pub fn withdraw_nonce_account<T: Config>(
	...
	let signer = match state.state() {
		...
		State::Initialized(ref data) => {
			if lamports == from.get_lamports() {
				...
				let amount = checked_add(lamports, min_balance.into())?;
	...
	if !signers.contains(&signer) {
		ic_msg!(invoke_context, "Withdraw nonce account: Account {} must sign", signer);
		return Err(InstructionError::MissingRequiredSignature);
	}
...
pub fn initialize_nonce_account<T: Config>(
	account: &mut BorrowedAccount<T>,
...
pub fn authorize_nonce_account<T: Config>(
	account: &mut BorrowedAccount<T>,
	nonce_authority: &Pubkey,
	signers: &HashSet<Pubkey>,
	invoke_context: &InvokeContext<T>,
	...
	match account.get_state::<Versions>()?.authorize(signers, *nonce_authority) {
...
mod test {
	...
	macro_rules! push_instruction_context {
		($invoke_context:expr, $transaction_context:ident, $instruction_context:ident,
		 $instruction_accounts:ident) => {
			$invoke_context
				.transaction_context
			...
			let $instruction_context =
				$transaction_context.get_current_instruction_context().unwrap();
		};
	}

	macro_rules! prepare_mockup {
		($invoke_context:ident, $instruction_accounts:ident, $rent:ident) => {
			let $rent = Rent { lamports_per_byte_year: 42, ..Rent::default() };
			let from_lamports = $rent.minimum_balance(State::size()) + 42;
			let transaction_accounts = vec![
				(Pubkey::new_unique(), create_account(from_lamports).into_inner()),
	...
	fn verify_nonce_ok() {
		...
		let mut nonce_account = instruction_context
			...
			.unwrap();
		let mut signers = HashSet::new();
		signers.insert(nonce_account.get_key().clone());
		...
		assert_matches!(
			verify_nonce_account(
				&transaction_context.get_account_at_index(NONCE_ACCOUNT_INDEX).unwrap().borrow(),
				DurableNonce::from_blockhash(&invoke_context.environment_config.blockhash)
					.as_hash(),
			),
			Some(_)
	...
	fn verify_nonce_bad_query_hash_fail() {
		...
		set_invoke_context_blockhash!(invoke_context, 0);
		let authorized = *nonce_account.get_key();
		initialize_nonce_account(&mut nonce_account, &authorized, &Rent::free(), &invoke_context)
			.unwrap();
