...
pub(crate) fn get_program_modification_slot<CB: TransactionProcessingCallback>(
	...
	if bpf_loader_upgradeable::check_id(program.owner()) {
		if let Ok(UpgradeableLoaderState::Program { programdata_address }) = program.state() {
	...
	*/
	...
	impl TransactionProcessingCallback for MockBankCallback {
		...
		fn add_builtin_account(&self, name: &str, program_id: &Pubkey) {
			...
			self.account_shared_data.borrow_mut().insert(*program_id, account_data);
		}
	}
	...
	fn test_load_program_accounts_account_not_found() {
		...
		let state = UpgradeableLoaderState::Program { programdata_address: Pubkey::new_unique() };
		account_data.set_data(bincode::serialize(&state).unwrap());
		mock_bank.account_shared_data.borrow_mut().insert(key, account_data.clone());

		let result = load_program_accounts(&mock_bank, &key);
		assert!(matches!(result, Some(ProgramAccountLoadResult::InvalidAccountData(_))));

		account_data.set_data(Vec::new());
		mock_bank.account_shared_data.borrow_mut().insert(key, account_data);
	...
	fn test_load_program_accounts_success() {
		...
		let state = UpgradeableLoaderState::Program { programdata_address: key2 };
		account_data.set_data(bincode::serialize(&state).unwrap());
		mock_bank.account_shared_data.borrow_mut().insert(key1, account_data.clone());

		let state =
			UpgradeableLoaderState::ProgramData { slot: 25, upgrade_authority_address: None };
		let mut account_data2 = AccountSharedData::default();
		account_data2.set_data(bincode::serialize(&state).unwrap());
		mock_bank.account_shared_data.borrow_mut().insert(key2, account_data2.clone());
	...
	fn test_load_program_from_bytes() {
		...
		let result =
			load_program_from_bytes(&mut metrics, &buffer, &loader, size, slot, environment, true);
	...
	fn test_load_program_not_found() {
		...
		let batch_processor = TransactionProcessor::default();
	...
	fn test_load_program_invalid_account_data() {
		...
		let loaded_program = ProgramCacheEntry::new_tombstone(
			...
			ProgramCacheEntryType::FailedVerification(
				batch_processor.get_environments_for_epoch(20).unwrap().program_runtime_v1,
			),
		);
	...
	fn test_load_program_program_loader_v3() {
		...
		let state =
			UpgradeableLoaderState::ProgramData { slot: 0, upgrade_authority_address: None };
		let mut account_data2 = AccountSharedData::default();
		account_data2.set_data(bincode::serialize(&state).unwrap());
		mock_bank.account_shared_data.borrow_mut().insert(key2, account_data2.clone());
		...
		let loaded_program = ProgramCacheEntry::new_tombstone(
			...
			ProgramCacheEntryType::FailedVerification(
				batch_processor.get_environments_for_epoch(0).unwrap().program_runtime_v1,
			),
		);
		assert_eq!(result.unwrap(), Arc::new(loaded_program));
		...
		mock_bank.account_shared_data.borrow_mut().insert(key2, account_data.clone());
	...
	fn test_load_program_environment() {
		...
		for is_upcoming_env in [false, true] {
			let result = load_program_with_pubkey(
				&mock_bank,
				&batch_processor.get_environments_for_epoch(is_upcoming_env as u64).unwrap(),
				&key,
				200,
				false,
			)
	...
	*/
	...
	fn test_program_modification_slot_success() {
		...
		let account_data = AccountSharedData::new_data(
			100,
			&UpgradeableLoaderState::Program { programdata_address: key2 },
			&bpf_loader_upgradeable::id(),
		)
		.unwrap();
		mock_bank.account_shared_data.borrow_mut().insert(key1, account_data);

		let account_data = AccountSharedData::new_data(
			100,
			&UpgradeableLoaderState::ProgramData { slot: 77, upgrade_authority_address: None },
			&bpf_loader_upgradeable::id(),
		)
		.unwrap();
