...
#[runtime_interface]
pub trait Crypto {
  fn webauthn_es256_verify(sig: &crate::p256::Signature, msg: &[u8], client_data_json: &[u8], authenticator_data: &[u8], pub_key: &[u8]) -> bool {
...
mod tests {
  ...
  fn parse_client_data_json_test() {
    ...
    let client_data: ClientDataJson = serde_json::from_slice(client_data_json.as_slice()).unwrap();
