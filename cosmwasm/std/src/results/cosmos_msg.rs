...
use crate::Decimal;
use crate::{coin::Coin, prelude::*, to_json_binary, Binary, StdResult};
...
impl<T> CosmosMsg<T> {
	/// Convert this this [`CosmosMsg<T>`] to a [`CosmosMsg<U>`] with a different custom message
	/// type. This allows easier interactions between code written for a specific chain and
...
pub enum StakingMsg {
	...
	/// `delegator_address` is automatically filled with the current contract's address.
	Redelegate { src_validator: String, dst_validator: String, amount: Coin },
}
...
pub enum GovMsg {
	...
	#[cfg(feature = "cosmwasm_1_2")]
	VoteWeighted { proposal_id: u64, options: Vec<WeightedVoteOption> },
}
...
/// code.
///
/// When using this, `admin` is always unset. If you need more flexibility, create the message
/// directly.
pub fn wasm_instantiate(
	...
	Ok(WasmMsg::Instantiate { admin: None, code_id, msg: payload, funds, label })
}
...
pub fn wasm_execute(
	...
	let payload = to_json_binary(msg)?;
	Ok(WasmMsg::Execute { contract_addr: contract_addr.into(), msg: payload, funds })
}
...
mod tests {
	...
	fn gov_msg_serializes_to_correct_json() {
		// Vote
		let msg = GovMsg::Vote { proposal_id: 4, option: VoteOption::NoWithVeto };
		let json = to_json_binary(&msg).unwrap();
		assert_eq!(
			String::from_utf8_lossy(&json),
		...
		#[cfg(feature = "cosmwasm_1_2")]
		{
			let msg = GovMsg::VoteWeighted {
				proposal_id: 25,
				options: vec![
					WeightedVoteOption { weight: Decimal::percent(25), option: VoteOption::Yes },
					WeightedVoteOption { weight: Decimal::percent(25), option: VoteOption::No },
					WeightedVoteOption {
	...
	fn change_custom_works() {
		...
		let send = BankMsg::Send { to_address: "you".to_string(), amount: coins(1015, "earth") };
		// Custom to Empty
