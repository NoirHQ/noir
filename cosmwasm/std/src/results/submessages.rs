...
pub struct SubMsg<T = Empty> {
	...
	///
	/// Setting this to `None` means unlimited. Then the submessage execution can consume all gas
...
impl<T> SubMsg<T> {
	...
	///
	/// By default, the submessage's gas limit will be unlimited. Use [`SubMsg::with_gas_limit`] to
	/// change it. Setting `payload` is not advised as this will never be used.
	...
	/// `Ok`.
	///
	/// By default, the submessage's `payload` will be empty and the gas limit will be unlimited.
	/// Use [`SubMsg::with_payload`] and [`SubMsg::with_gas_limit`] to change those.
	...
	fn reply_on(msg: CosmosMsg<T>, id: u64, reply_on: ReplyOn) -> Self {
		SubMsg { id, payload: Default::default(), msg, reply_on, gas_limit: None }
	}
...
pub struct SubMsgResponse {
	...
	/// In most cases, this is equivalent to the Cosmos SDK's [MsgResponses], which usually
	/// contains a [single message]. However, wasmd allows chains to translate a single contract
	/// message into multiple SDK messages. In that case all the MsgResponses from each are
	/// concatenated into this flattened `Vec`.
	///
...
mod tests {
	...
	fn sub_msg_result_serialization_works() {
		let result =
			SubMsgResult::Ok(SubMsgResponse { data: None, msg_responses: vec![], events: vec![] });
		assert_eq!(
	...
	fn sub_msg_result_can_convert_from_core_result() {
		...
		let converted: SubMsgResult = original.into();
		assert_eq!(converted, SubMsgResult::Err("Generic error: broken".to_string()));
	}
