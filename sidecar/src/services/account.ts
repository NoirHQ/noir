...
export class NoirAccountService implements IAccountService {
  ...
  public async origin(address: string): Promise<Codec> {
    const { data } = fromBech32(address);
    const addressRaw = Buffer.concat([Buffer.from([0x01]), data]);
  ...
  public interim(address: string): string {
    const { data } = fromBech32(address);
    const addressRaw = blake2b(Buffer.concat([Buffer.from("cosm:", "utf8"), data]), 32);
    return `0x${Buffer.from(addressRaw).toString("hex")}`;
  }
}
